...
func save_level_progress(
	max_level: int, current_level: int, level_highscores: Dictionary = {}
) -> bool:
...
func save_level_high_score(level: int, difficulty: String, score: int) -> bool:
	...
	if score > current_high_score:
		...
		if (
			not game_state["global_highscores"].has(difficulty)
			or score > game_state["global_highscores"][difficulty]
		):
