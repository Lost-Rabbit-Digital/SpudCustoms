...
static func create_passport_stampable(game_scene: Node, stamp_system: StampSystem) -> StampableComponent:
	...
	if !document_sprite:
		...
		var passport_rect = Rect2(
			...
			Vector2(600, 300)  # Approximate size based on your screenshot
		)
		
		# Target the red highlighted visa area from your screenshot
		var visa_area = Rect2(
			passport_rect.position + Vector2(passport_rect.size.x * 0.25, passport_rect.size.y * 0.25), 
		...
		var sprite_global_rect = Rect2(
			document_sprite.global_position - (document_sprite.texture.get_size() * document_sprite.scale / 2),
		...
		var visa_area = Rect2(
			sprite_global_rect.position + Vector2(sprite_global_rect.size.x * 0.3, sprite_global_rect.size.y * 0.2),
