...
func register_stampable(
	document: Node2D, content_node: Node, stamp_area: Rect2 = Rect2()
) -> StampableComponent:
...
func on_stamp_requested(
	stamp_type: String, stamp_texture: Texture2D = null, source_node: Node = null
):
...
func apply_stamp(stamp_type: String, stamp_bar: Node = null):
	...
	tween.tween_property(
		stamp_button,
		"global_position",
		Vector2(stamp_button.global_position.x, stamp_button.global_position.y + STAMP_OFFSET),
		STAMP_ANIM_DURATION / 2
	)
	tween.tween_property(
		stamp_button, "scale", original_scale * randf_range(0.9, 1.1), STAMP_ANIM_DURATION / 2
	)
	tween.tween_property(
		stamp_button,
		"rotation",
		original_rotation + randf_range(-0.1, 0.1),
		STAMP_ANIM_DURATION / 2
	)
	...
	(
		second_tween
		. tween_callback(
			_apply_stamp_impact.bind(stamp_button, original_scale, fixed_stamp_position, stamp_type)
		)
		. set_delay(STAMP_ANIM_DURATION / 2)
	)  # Delay to match first tween duration

	# Return animation with slight bounce effect
	(
		second_tween
		. tween_callback(
			_return_stamp_to_position.bind(
				stamp_button,
				original_position,
				original_scale,
				original_rotation,
		...
		. set_delay(STAMP_ANIM_DURATION / 6)
	)  # Short delay for the press effect


# Handle the stamp impact animation and effects
func _apply_stamp_impact(
	stamp_button: TextureButton,
	original_scale: Vector2,
	fixed_stamp_position: Vector2,
	stamp_type: String
) -> void:
...
# Handle the return animation of the stamp to its original position
func _return_stamp_to_position(
	stamp_button: TextureButton,
	original_position: Vector2,
	original_scale: Vector2,
	original_rotation: float,
	original_z_index: int,
	original_mouse_filter: int,
	stamp_type: String
) -> void:
	var return_tween = create_tween()
	return_tween.set_parallel(true)
	return_tween.tween_property(
		stamp_button, "global_position", original_position, STAMP_ANIM_DURATION / 2
	)
	(
		return_tween
		. tween_property(stamp_button, "scale", original_scale, STAMP_ANIM_DURATION / 2)
		. set_trans(Tween.TRANS_BOUNCE)
	)
	return_tween.tween_property(
		stamp_button, "rotation", original_rotation, STAMP_ANIM_DURATION / 2
	)

	# Restore original properties and set state when done
	return_tween.tween_callback(
		_complete_stamp_animation.bind(
			stamp_button, original_z_index, original_mouse_filter, stamp_type
		)
	)


# Complete stamp animation and reset state
func _complete_stamp_animation(
	stamp_button: TextureButton,
	original_z_index: int,
	original_mouse_filter: int,
	stamp_type: String
