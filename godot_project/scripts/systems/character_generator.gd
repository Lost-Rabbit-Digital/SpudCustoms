		...
		if head.sprite_frames.has_animation(head_anim):
			...
			push_error(
				(
					...
					+ str(head.sprite_frames.get_animation_names())
				)
			)
		...
		if face.sprite_frames.has_animation(face_anim):
			...
			push_error(
				(
					...
					+ face_anim
					+ " - Available animations: "
					+ str(face.sprite_frames.get_animation_names())
				)
			)
		...
		if torso.sprite_frames.has_animation(torso_anim):
			...
			push_error(
				(
					...
					+ torso_anim
					+ " - Available animations: "
					+ str(torso.sprite_frames.get_animation_names())
				)
			)
...
func _validate_animations():
	...
	if head and head.animation != expected_head_anim:
		push_warning(
			(
				"Head animation mismatch! Current: "
				+ head.animation
				+ ", Expected: "
				+ expected_head_anim
			)
	...
	if face and face.animation != expected_face_anim:
		push_warning(
			(
				"Face animation mismatch! Current: "
				+ face.animation
				+ ", Expected: "
				+ expected_face_anim
			)
	...
	if torso and torso.animation != expected_body_anim:
		push_warning(
			(
				"Body animation mismatch! Current: "
				+ torso.animation
				+ ", Expected: "
				+ expected_body_anim
			)
...
func fade_out_foreground_shadow(duration: float = 1.5):
	...
	(
		tween
		. tween_property(foreground_shadow, "modulate", transparent_color, duration)
...
func fade_in_foreground_shadow(duration: float = 1.5):
	...
	(
		tween
		. tween_property(foreground_shadow, "modulate", opaque_color, duration)
		. set_trans(Tween.TRANS_SINE)
		. set_ease(Tween.EASE_OUT)
	)
