...
var active_shutter_state = ShutterState.CLOSED
...
func _ready():
	...
	if lever_sprite:
		...
		lever_sprite.frame = 7 if active_shutter_state == ShutterState.CLOSED else 0
...
func raise_shutter(duration: float = 0.5):
	active_shutter_state = ShutterState.OPEN
	...
	(
		tween
		. tween_property(
			shutter, "position", Vector2(shutter.position.x, end_node.position.y), duration
		)
...
func lower_shutter(_duration: float = 3.0):
	active_shutter_state = ShutterState.CLOSED
	...
	(
		tween
		. tween_property(
			shutter, "position", Vector2(shutter.position.x, start_node.position.y + 0.8), 0.7  # Drop time
		)
	...
	(
		tween
		. tween_property(
			shutter, "position", Vector2(shutter.position.x, start_node.position.y - 35), 0.25  # Y / Time
		)
	...
	(
		tween
		. tween_property(shutter, "position", start_node.position, 0.25)  # Settling time
...
func animate_lever(to_up: bool):
	...
	lever_sfx.finished.connect(func(): lever_sfx.queue_free())
...
func shutter_state_toggle() -> void:
	...
	if active_shutter_state == ShutterState.CLOSED:
