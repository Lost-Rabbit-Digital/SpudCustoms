...
func check_stamp_accuracy(stamp_pos: Vector2, passport: Node2D) -> bool:
	...
	var visa_rect = Rect2(passport.global_position + Vector2(20, 20), Vector2(160, 80))  # Adjust based on your visa position  # Adjust based on your visa size

	# Get stamp rectangle
	var stamp_rect = Rect2(stamp_pos - Vector2(25, 25), Vector2(50, 50))  # Half stamp size  # Stamp size
