...
func initialize_ui() -> void:
	...
	add_theme_constant_override(
		"margin_bottom", DialogicUtil.get_editor_setting("event_block_margin", 0) * _scale
	)
	...
	%ToggleBodyVisibilityButton.set(
		"theme_override_colors/icon_normal_color", get_theme_color("contrast_color_2", "Editor")
	)
	%ToggleBodyVisibilityButton.set(
		"theme_override_colors/icon_hover_color", get_theme_color("accent_color", "Editor")
	)
	%ToggleBodyVisibilityButton.set(
		"theme_override_colors/icon_pressed_color", get_theme_color("contrast_color_2", "Editor")
	)
	%ToggleBodyVisibilityButton.set(
		"theme_override_colors/icon_hover_pressed_color", get_theme_color("accent_color", "Editor")
	)
...
func visual_select() -> void:
	$PanelContainer.add_theme_stylebox_override(
		"panel", load("res://addons/dialogic/Editor/Events/styles/selected_styleboxflat.tres")
	)
	selected = true
	%IconPanel.self_modulate = resource.event_color
...
func visual_deselect() -> void:
	$PanelContainer.add_theme_stylebox_override(
		"panel", load("res://addons/dialogic/Editor/Events/styles/unselected_stylebox.tres")
	)
	selected = false
...
func set_indent(indent: int) -> void:
	add_theme_constant_override(
		"margin_left", indent_size * indent * DialogicUtil.get_editor_scale()
	)
...
var FIELD_SCENES := {
	DialogicEvent.ValueType.MULTILINE_TEXT:
	"res://addons/dialogic/Editor/Events/Fields/field_text_multiline.tscn",
	DialogicEvent.ValueType.SINGLELINE_TEXT:
	"res://addons/dialogic/Editor/Events/Fields/field_text_singleline.tscn",
	DialogicEvent.ValueType.FILE: "res://addons/dialogic/Editor/Events/Fields/field_file.tscn",
	DialogicEvent.ValueType.BOOL:
	"res://addons/dialogic/Editor/Events/Fields/field_bool_check.tscn",
	DialogicEvent.ValueType.BOOL_BUTTON:
	"res://addons/dialogic/Editor/Events/Fields/field_bool_button.tscn",
	DialogicEvent.ValueType.CONDITION:
	"res://addons/dialogic/Editor/Events/Fields/field_condition.tscn",
	DialogicEvent.ValueType.ARRAY: "res://addons/dialogic/Editor/Events/Fields/field_array.tscn",
	DialogicEvent.ValueType.DICTIONARY:
	"res://addons/dialogic/Editor/Events/Fields/field_dictionary.tscn",
	DialogicEvent.ValueType.DYNAMIC_OPTIONS:
	"res://addons/dialogic/Editor/Events/Fields/field_options_dynamic.tscn",
	DialogicEvent.ValueType.FIXED_OPTIONS:
	"res://addons/dialogic/Editor/Events/Fields/field_options_fixed.tscn",
	DialogicEvent.ValueType.NUMBER: "res://addons/dialogic/Editor/Events/Fields/field_number.tscn",
	DialogicEvent.ValueType.VECTOR2:
	"res://addons/dialogic/Editor/Events/Fields/field_vector2.tscn",
	DialogicEvent.ValueType.VECTOR3:
	"res://addons/dialogic/Editor/Events/Fields/field_vector3.tscn",
	DialogicEvent.ValueType.VECTOR4:
	"res://addons/dialogic/Editor/Events/Fields/field_vector4.tscn",
	DialogicEvent.ValueType.COLOR: "res://addons/dialogic/Editor/Events/Fields/field_color.tscn",
	DialogicEvent.ValueType.AUDIO_PREVIEW:
	"res://addons/dialogic/Editor/Events/Fields/field_audio_preview.tscn",
	DialogicEvent.ValueType.IMAGE_PREVIEW:
	"res://addons/dialogic/Editor/Events/Fields/field_image_preview.tscn",
}
...
func build_editor(build_header: bool = true, build_body: bool = false) -> void:
	...
	for p in resource.get_event_editor_info():
		...
		if p.name == "linebreak":
			...
			editor_node.add_theme_color_override(
				"font_color",
				resource.event_color.lerp(get_theme_color("font_color", "Editor"), 0.8)
			)
		...
		if !p.get("right_text", "").is_empty():
			...
			right_label.add_theme_color_override(
				"font_color",
				resource.event_color.lerp(get_theme_color("font_color", "Editor"), 0.8)
			)
		...
		if p.has("condition"):
			...
			if left_label:
				field_list.append(
					{"node": left_label, "condition": p.condition, "location": p.location}
				)
			if right_label:
				field_list.append(
					{"node": right_label, "condition": p.condition, "location": p.location}
				)
...
func _evaluate_visibility_condition(p: Dictionary) -> bool:
	...
	if expr.has_execute_failed():
		printerr(
			"[Dialogic] Failed executing visibility condition for '",
			p.get("property", "unnamed"),
			"': " + expr.get_error_text()
		)
	return result
