...
func add_portrait_item(
	portrait_name: String, portrait_data: Dictionary, parent_item: TreeItem, previous_name := ""
) -> TreeItem:
...
func add_portrait_group(
	goup_name := "Group", parent_item: TreeItem = get_root(), previous_name := ""
) -> TreeItem:
	var item: TreeItem = %PortraitTree.create_item(parent_item)
...
func copy_branch_or_item(item: TreeItem, new_parent: TreeItem) -> TreeItem:
	...
	if item.get_metadata(0).has("group"):
		...
		new_item = add_portrait_item(
			item.get_text(0), item.get_metadata(0), new_parent, item.get_meta("previous_name")
		)
