...
func _ready() -> void:
	...
	(
		...
		. connect(_on_resources_reimported)
	)
...
func load_info(info: Dictionary, update_type: int) -> void:
	...
	if update_type == 2:
		...
		%UpdateName.add_theme_color_override(
			"font_color", editor_view.get_theme_color("readonly_color", "Editor")
		)
	...
	if info.is_empty():
		...
		%UpdateName.add_theme_color_override(
			"font_color", editor_view.get_theme_color("property_color_z", "Editor")
		)
		%Install.disabled = true
		...
		%UpdateName.add_theme_color_override(
			"font_color", editor_view.get_theme_color("warning_color", "Editor")
		)
		%Install.disabled = false
	else:
		%State.text = "You are up to date:"
		%UpdateName.add_theme_color_override(
			"font_color", editor_view.get_theme_color("success_color", "Editor")
		)
		%Install.disabled = true
	...
	%ShortInfo.text = (
		"Published on "
		+ info.published_at.substr(0, info.published_at.find("T"))
		+ " by "
		+ info.author.login
	)
...
func markdown_to_bbcode(text: String) -> String:
	...
	while res:
		text = text.replace(
			...
			(
				"[font_size="
				+ str(font_sizes[len(res.get_string("level"))])
				+ "]"
				+ res.get_string("title").strip_edges()
				+ "[/font_size]"
			)
		)
	...
	while res:
		text = text.replace(
			res.get_string(), "[ul]" + res.get_string("text").strip_edges() + "[/ul]\n"
		)
		res = bullets_regex.search(text)
