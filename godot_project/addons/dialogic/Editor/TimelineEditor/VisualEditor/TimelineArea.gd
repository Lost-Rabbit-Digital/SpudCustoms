...
func _process(delta: float) -> void:
	...
	for child in %Timeline.get_children():
		if (
			(child.global_position.y < get_global_mouse_position().y)
			and (child.global_position.y + child.size.y > get_global_mouse_position().y)
		):
...
func _draw() -> void:
	...
	for idx in range($Timeline.get_child_count()):
		...
		var rect_position: Vector2 = (
			block.get_node("%IconPanel").global_position
			+ Vector2(0, 1) * block.get_node("%IconPanel").size
			+ Vector2(0, -4)
		)
		...
		if block.expanded and not block.resource.can_contain_events:
			draw_rect(
				Rect2(
					rect_position - global_position + Vector2(line_width, 0),
					Vector2(line_width, block.size.y - block.get_node("%IconPanel").size.y)
				),
				color
			)
		...
		if (
			idx >= $Timeline.get_child_count() - 1
			or block.current_indent_level >= $Timeline.get_child(idx + 1).current_indent_level
		):
			continue
		...
		if block.resource.can_contain_events:
			...
			if end_node != null:
				var v_length: float = (
					end_node.global_position.y + end_node.size.y / 2 - rect_position.y
				)
				...
				# Draw vertical line
				draw_rect(
					Rect2(rect_position - global_position + offset, Vector2(line_width, v_length)),
					color
				)
		...
		if block.resource.wants_to_group:
			...
			if idx != 0:
				...
				if block_above.resource.event_name == block.resource.event_name:
					group_starter = false
				if (
					block_above.resource is DialogicEndBranchEvent
					and block_above.parent_node.resource.event_name == block.resource.event_name
				):
					group_starter = false
			...
			if group_starter:
				...
				while sub_idx < $Timeline.get_child_count() - 1:
					sub_idx += 1
					if (
						$Timeline.get_child(sub_idx).current_indent_level
						== block.current_indent_level - 1
					):
				...
				var v_length: float = (
					end_node.global_position.y - rect_position.y + icon_panel_height
				)
	...
	if dragging and get_global_rect().has_point(get_global_mouse_position()):
		...
		if drag_to_position == %Timeline.get_child_count():
			height = (
				%Timeline.get_child(-1).global_position.y
				+ %Timeline.get_child(-1).size.y
				- global_position.y
				- (line_width / 2.0)
			)
		else:
			height = (
				%Timeline.get_child(drag_to_position).global_position.y
				- global_position.y
				- (line_width / 2.0)
			)
