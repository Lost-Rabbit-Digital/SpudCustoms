...
func _ready() -> void:
	...
	%IgnoredCharactersEnabled.toggled.connect(
		_on_bool_set.bind(_SETTING_AUTOADVANCE_IGNORED_CHARACTERS_ENABLED)
	)

	%AutoskipTimePerEvent.value_changed.connect(
		_on_float_set.bind(_SETTING_AUTOSKIP_TIME_PER_EVENT)
	)
...
func _refresh() -> void:
	...
	%InputAction.set_value(
		ProjectSettings.get_setting(_SETTING_INPUT_ACTION, &"dialogic_default_action")
	)
	...
	var per_character_delay: float = ProjectSettings.get_setting(
		_SETTING_AUTOADVANCE_CHARACTER_DELAY, 0.1
	)
	...
	%IgnoredCharactersEnabled.button_pressed = ProjectSettings.get_setting(
		_SETTING_AUTOADVANCE_IGNORED_CHARACTERS_ENABLED, true
	)
	var ignored_characters: String = ""
	var ignored_characters_dict: Dictionary = ProjectSettings.get_setting(
		_SETTING_AUTOADVANCE_IGNORED_CHARACTERS, {}
	)
	...
	## AUTO-PAUSES
	%AutoPausesAbsolute.button_pressed = ProjectSettings.get_setting(
		_SETTING_ABSOLUTE_AUTOPAUSES, false
	)
		...
		2:  # PER CHARACTER
			...
			if delay != -1:
				...
				ProjectSettings.set_setting(
					_SETTING_AUTOADVANCE_CHARACTER_DELAY, %AdditionalDelay.value
				)
				ProjectSettings.set_setting(_SETTING_AUTOADVANCE_WORD_DELAY, 0)
	ProjectSettings.save()
...
func _on_IgnoredCharacters_text_changed(text_input):
	ProjectSettings.set_setting(
		_SETTING_AUTOADVANCE_IGNORED_CHARACTERS, DialogicUtil.str_to_hash_set(text_input)
	)
	ProjectSettings.save()
