...
func _ready() -> void:
	...
	%ChangeTypePopup.theme.set_stylebox(
		"pressed", "Button", get_theme_stylebox("LaunchPadMovieMode", "EditorStyles")
	)
	%ChangeTypePopup.theme.set_stylebox(
		"hover", "Button", get_theme_stylebox("LaunchPadMovieMode", "EditorStyles")
	)
	for child in %ChangeTypePopup/HBox.get_children():
		child.toggled.connect(_on_type_pressed.bind(child.get_index() + 1))
		child.icon = get_theme_icon(
			["String", "float", "int", "bool"][child.get_index()], "EditorIcons"
		)
...
func add_folder_item(item_name: String, parent: TreeItem) -> TreeItem:
	...
	if parent == null:
		...
		var parent_color: Color = parent.get_meta(
			"color", Color(0.33000001311302, 0.15179999172688, 0.15179999172688)
		)
	...
	if parent:
		parent_vars_count = parent.get_children().reduce(
			func(x, y): return x + 1 if y.get_meta("type") == "VARIABLE" else x, 0
		)

	if level == 2:
		folder_color.h = wrap(
			folder_color.h + 0.15 * (item.get_index() - parent_vars_count + 1), 0, 1
		)
		#folder_color = folder_color.lerp(get_theme_color("background", "Editor"), 0.1*level)
	elif level == 3:
		folder_color.h = wrap(
			folder_color.h - 0.1 * (item.get_index() - parent_vars_count + 1), 0, 1
		)
	elif level == 4:
		folder_color.h = wrap(
			folder_color.h + 0.1 * (item.get_index() - parent_vars_count + 1), 0, 1
	...
	item.add_button(
		2,
		load(self.get_script().get_path().get_base_dir().get_base_dir() + "/add-variable.svg"),
		TreeButtons.ADD_VARIABLE
	)
	item.add_button(
		2, load("res://addons/dialogic/Editor/Images/Pieces/add-folder.svg"), TreeButtons.ADD_FOLDER
	)
	item.add_button(
		2,
	...
	item.add_button(
		2, get_theme_icon("Remove", "EditorIcons"), TreeButtons.DELETE, item == get_root()
	)
...
func set_variable_item_type(item: TreeItem, type: int) -> void:
	...
	item.set_button(
		1, 0, get_theme_icon(["Variant", "String", "float", "int", "bool"][type], "EditorIcons")
	)
...
func _on_button_clicked(item: TreeItem, _column: int, id: int, _mouse_button_index: int) -> void:
	match id:
		...
		TreeButtons.CHANGE_TYPE:
			...
			%ChangeTypePopup.position = (
				get_local_mouse_position() + Vector2(-%ChangeTypePopup.size.x / 2, 10)
			)
			for child in %ChangeTypePopup/HBox.get_children():
				child.set_pressed_no_signal(false)
			(
				%ChangeTypePopup/HBox
				. get_child(int(item.get_meta("value_type", DialogicUtil.VarTypes.STRING) - 1))
...
func item_add_undoable(
	parent_item: TreeItem, item_name: String, type: String, value: Variant
) -> void:
	undo.create_action("Add Item")
	undo.add_do_method(add_item.bind(get_item_path(parent_item), item_name, type, value))
...
func item_remove_undoable(item: TreeItem) -> void:
	...
	undo.add_undo_method(
		add_item.bind(
			...
			item.get_meta("type"),
			(
				get_variable_item_default(item)
...
func filter(filter_term: String, item: TreeItem = null) -> bool:
	...
	for child in item.get_children():
		match child.get_meta("type"):
			"VARIABLE":
				child.visible = (
					filter_term.is_empty() or filter_term.to_lower() in child.get_text(0).to_lower()
				)
