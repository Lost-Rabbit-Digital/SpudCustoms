...
func _apply_export_overrides() -> void:
	...
	if history_subsystem != null:
		get_show_history_button().visible = (
			show_open_button and history_subsystem.get(&"simple_history_enabled")
		)
	else:
		set(&"visible", false)
	...
	if ResourceLoader.exists(font_custom_italics):
		history_item_theme.set_font(
			&"RichtTextLabel", &"italics_font", load(font_custom_italics) as Font
		)
				...
				if info.has("character") and info["character"]:
					if show_name_colors:
						history_item.call(
							&"load_info",
							info["text"],
							info["character"] + name_delimeter,
							info["character_color"]
						)
					else:
						history_item.call(
							&"load_info", info["text"], info["character"] + name_delimeter
						)
				else:
