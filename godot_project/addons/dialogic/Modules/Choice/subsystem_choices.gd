...
func _ready() -> void:
	...
	reveal_by_input = ProjectSettings.get_setting(
		"dialogic/choices/reveal_by_input", reveal_by_input
	)
	block_delay = ProjectSettings.get_setting("dialogic/choices/delay", block_delay)
	autofocus_first_choice = ProjectSettings.get_setting(
		"dialogic/choices/autofocus_first", autofocus_first_choice
	)
	hotkey_behaviour = ProjectSettings.get_setting(
		"dialogic/choices/hotkey_behaviour", hotkey_behaviour
	)
	default_false_behaviour = ProjectSettings.get_setting(
		"dialogic/choices/def_false_behaviour", default_false_behaviour
	)
	...
	for choice_index in get_current_choice_indexes():
		...
		if condition.is_empty() or dialogic.Expressions.execute_condition(choice_event.condition):
			...
			hide = (
				hide
				or (
					choice_event.else_action == DialogicChoiceEvent.ElseActions.DEFAULT
					and default_false_behaviour == DialogicChoiceEvent.ElseActions.HIDE
				)
			)
...
func show_current_question(instant := true) -> void:
	...
	if missing_button:
		printerr(
			"[Dialogic] The layout you are using doesn't have enough Choice Buttons for the choices you are trying to display."
		)
	...
	if dialogic.has_subsystem("History"):
		...
		if dialogic.has_subsystem("VAR"):
			dialogic.History.store_simple_history_entry(
				dialogic.VAR.parse_variables(choice_info.text),
				"Choice",
				{"all_choices": all_choices}
			)
		else:
			dialogic.History.store_simple_history_entry(
				choice_info.text, "Choice", {"all_choices": all_choices}
			)
		if dialogic.has_subsystem("History"):
...
func is_question(index: int) -> bool:
	...
	if event is DialogicChoiceEvent:
		...
		if not prev_event is DialogicEndBranchEvent:
			return true
		var prev_event_opener: DialogicEvent = dialogic.current_timeline_events[
			prev_event.get_opening_index()
		]
