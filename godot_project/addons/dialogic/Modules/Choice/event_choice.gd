...
var regex := (
	RegEx
	. create_from_string(
		r"- (?<text>(?>\\\||(?(?=.*\|)[^|]|(?!\[if)[^|]))*)\|?\s*(\[if(?<condition>([^\]\[]|\[[^\]]*\])+)\])?\s*(\[(?<shortcode>[^]]*)\])?"
	)
)
...
func _get_end_branch_control() -> Control:
	return (
		load(get_script().resource_path.get_base_dir().path_join("ui_choice_end.tscn"))
		. instantiate()
	)
...
func build_event_editor() -> void:
	...
	add_body_edit(
		...
		{"editor_icon": ["Add", "EditorIcons"], "tooltip": "Add Condition"},
		"not _has_condition"
	)
	...
	add_body_edit(
		"_has_condition",
		ValueType.BOOL_BUTTON,
		{"editor_icon": ["Remove", "EditorIcons"], "tooltip": "Remove Condition"},
		"_has_condition"
	)
	add_body_edit(
		"else_action",
	...
	add_body_edit(
		...
		{"left_text": "Disabled text:", "placeholder": "(Empty for same)"},
		"allow_alt_text()"
	)
	add_body_line_break()
...
func allow_alt_text() -> bool:
	return (
		_has_condition
		and (
			else_action == ElseActions.DISABLE
			or (
				else_action == ElseActions.DEFAULT
				and ProjectSettings.get_setting("dialogic/choices/def_false_behaviour", 0) == 1
			)
		)
	)
	...
	if symbol == "[":
		if !"[if" in line and line.count("[") - line.count("]") == 1:
			TextNode.add_code_completion_option(
				CodeEdit.KIND_MEMBER, "if", "if ", TextNode.syntax_highlighter.code_flow_color
			)
		elif "[if" in line:
			TextNode.add_code_completion_option(
				CodeEdit.KIND_MEMBER, "else", 'else="', TextNode.syntax_highlighter.code_flow_color
			)
...
func _get_syntax_highlighting(
	...
	dict = Highlighter.color_region(
		dict,
		event_color.lerp(Highlighter.variable_color, 0.5),
		line,
		"{",
		"}",
		0,
