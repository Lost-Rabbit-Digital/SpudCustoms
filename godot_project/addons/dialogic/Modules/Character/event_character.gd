...
func _execute() -> void:
	...
	if action == Actions.JOIN:
		if dialogic.has_subsystem("History") and !dialogic.Portraits.is_character_joined(character):
			...
			dialogic.History.store_simple_history_entry(
				character_name_text + " joined",
				event_name,
				{"character": character_name_text, "mode": "Join"}
			)
	...
	# LEAVE -------------------------------------
		...
		if character_identifier == "--All--":
			if (
				dialogic.has_subsystem("History")
				and len(dialogic.Portraits.get_joined_characters())
			):
				dialogic.History.store_simple_history_entry(
					"Everyone left", event_name, {"character": "All", "mode": "Leave"}
				)
			...
			if (
				dialogic.has_subsystem("History")
				and dialogic.Portraits.is_character_joined(character)
			):
				var character_name_text := dialogic.Text.get_character_name_parsed(character)
				dialogic.History.store_simple_history_entry(
					character_name_text + " left",
					event_name,
					{"character": character_name_text, "mode": "Leave"}
				)

			await dialogic.Portraits.leave_character(
				character, animation_name, final_animation_length, animation_wait
			)
	...
	# UPDATE -------------------------------------
		...
		if set_portrait:
			await dialogic.Portraits.change_character_portrait(
				character, portrait, fade_animation, fade_length
			)
		...
		if set_transform:
			dialogic.Portraits.move_character(
				character, transform, final_position_move_time, transform_ease, transform_trans
			)
...
func get_shortcode_parameters() -> Dictionary:
	return {
		...
		"move_ease":
		{
			"property": "transform_ease",
			"default": Tween.EaseType.EASE_IN_OUT,
			"suggestions": func(): return list_to_suggestions(ease_options)
		},
		"move_trans":
		{
			"property": "transform_trans",
			"default": Tween.TransitionType.TRANS_SINE,
...
func build_event_editor() -> void:
	...
	add_header_edit(
		"set_portrait",
		ValueType.BOOL_BUTTON,
		{
			"icon": load("res://addons/dialogic/Modules/Character/update_portrait.svg"),
			"tooltip": "Change Portrait"
		},
		"should_show_portrait_selector() and action == Actions.UPDATE"
	)
	add_header_edit(
		"portrait",
		ValueType.DYNAMIC_OPTIONS,
	...
	add_header_edit(
		...
		ValueType.BOOL_BUTTON,
		{
			"icon": load("res://addons/dialogic/Modules/Character/update_position.svg"),
			"tooltip": "Change Position"
		},
		"character != null and !has_no_portraits() and action == Actions.UPDATE"
	)
	add_header_label(
		"at position", "character != null and !has_no_portraits() and action == Actions.JOIN"
	)
	add_header_label(
		"to position",
		"character != null and !has_no_portraits() and action == Actions.UPDATE and set_transform"
	)
	add_header_edit(
		"transform",
		ValueType.DYNAMIC_OPTIONS,
		{
			...
			"tooltip":
			"You can use a predefined position or a custom transform like 'pos=x0.5y1 size=x0.5y1 rot=10'.\nLearn more about this in the documentation."
		},
		(
			"character != null and !has_no_portraits() and action != %s and (action != Actions.UPDATE or set_transform)"
			% Actions.LEAVE
		)
	)
	...
	add_body_edit(
		"fade_length",
		ValueType.NUMBER,
		{"left_text": "Length:", "suffix": "s", "min": 0},
	...
	add_body_edit(
		"animation_repeats",
		ValueType.NUMBER,
		{"left_text": "Repeat:", "mode": 1, "min": 1},
		(
			"should_show_animation_options() and !animation_name.is_empty() and action == %s)"
			% Actions.UPDATE
		)
	)
	add_body_line_break()
	add_body_edit(
		"transform_time",
		ValueType.NUMBER,
		{"left_text": "Movement duration:", "min": 0},
		"should_show_transform_options()"
	)
	add_body_edit(
		"transform_trans",
		ValueType.FIXED_OPTIONS,
		{"options": trans_options, "left_text": "Trans:"},
		"should_show_transform_options() and transform_time > 0"
	)
	add_body_edit(
		"transform_ease",
		ValueType.FIXED_OPTIONS,
		{"options": ease_options, "left_text": "Ease:"},
		"should_show_transform_options() and transform_time > 0"
	)
	...
	add_body_edit(
		...
		ValueType.BOOL_BUTTON,
		{
			"icon": load("res://addons/dialogic/Modules/Character/update_z_index.svg"),
			"tooltip": "Change Z-Index"
		},
		"character != null and action == Actions.UPDATE"
	)
	add_body_edit(
		"z_index",
		ValueType.NUMBER,
		{"left_text": "Z-index:", "mode": 1},
	...
	add_body_edit(
		...
		ValueType.BOOL_BUTTON,
		{
			"icon": load("res://addons/dialogic/Modules/Character/update_mirror.svg"),
			"tooltip": "Change Mirroring"
		},
		"character != null and action == Actions.UPDATE"
	)
	add_body_edit(
...
func should_show_fade_options() -> bool:
	return (
		...
		and set_portrait
		and character
		and not character.portraits.is_empty()
	)
...
func get_character_suggestions(search_text: String) -> Dictionary:
	return DialogicUtil.get_character_suggestions(
		search_text, character, false, action == Actions.LEAVE, editor_node
	)
...
func get_animation_suggestions(search_text: String = "") -> Dictionary:
	...
	match action:
		Actions.JOIN:
			return DPAU.get_suggestions(
				search_text, animation_name, "Default", DPAU.AnimationType.IN
			)
		Actions.LEAVE:
			return DPAU.get_suggestions(
				search_text, animation_name, "Default", DPAU.AnimationType.OUT
			)
		Actions.UPDATE:
			return DPAU.get_suggestions(
				search_text, animation_name, "None", DPAU.AnimationType.ACTION
			)
	return {}


func get_fade_suggestions(search_text: String = "") -> Dictionary:
	return DialogicPortraitAnimationUtil.get_suggestions(
		search_text,
		fade_animation,
		"Default",
		DialogicPortraitAnimationUtil.AnimationType.CROSSFADE
	)
...
func _get_code_completion(
	CodeCompletionHelper: Node, TextNode: TextEdit, line: String, _word: String, symbol: String
) -> void:
	...
	if symbol == " " and line_until_caret.count(" ") == 1:
		...
		if line.begins_with("leave"):
			TextNode.add_code_completion_option(
				...
				"--All-- ",
				event_color,
				TextNode.get_theme_icon("GuiEllipsis", "EditorIcons")
			)
	...
	if symbol == "(":
		...
		not "[" in line_until_caret
		and symbol == " "
		and line_until_caret.split(" ", false).size() > 1
	):
			...
			if not line_until_caret.split(" ", false)[-1] in get_position_suggestions():
				for position in get_position_suggestions():
					TextNode.add_code_completion_option(
						...
						position,
						position + " ",
						TextNode.syntax_highlighter.normal_color
					)
	...
	if "[" in line_until_caret:
		...
		# Suggest Values
			...
			var current_param: RegExMatch = (
				CodeCompletionHelper.completion_shortcode_param_getter_regex.search(line)
			)
			...
			match current_param.get_string("param"):
				"animation":
					...
					if line.begins_with("join"):
						animations = DialogicPortraitAnimationUtil.get_portrait_animations_filtered(
							DialogicPortraitAnimationUtil.AnimationType.IN
						)
					elif line.begins_with("update"):
						animations = DialogicPortraitAnimationUtil.get_portrait_animations_filtered(
							DialogicPortraitAnimationUtil.AnimationType.ACTION
						)
					elif line.begins_with("leave"):
						animations = DialogicPortraitAnimationUtil.get_portrait_animations_filtered(
							DialogicPortraitAnimationUtil.AnimationType.OUT
						)

					for script: String in animations:
						TextNode.add_code_completion_option(
							CodeEdit.KIND_VARIABLE,
							DialogicUtil.pretty_name(script),
							DialogicUtil.pretty_name(script),
							TextNode.syntax_highlighter.normal_color,
							null,
							'" '
				...
				"wait", "mirrored":
					CodeCompletionHelper.suggest_bool(
						TextNode, TextNode.syntax_highlighter.normal_color
					)
				"move_trans":
					CodeCompletionHelper.suggest_custom_suggestions(
						list_to_suggestions(trans_options),
						TextNode,
						TextNode.syntax_highlighter.normal_color
					)
				"move_ease":
					CodeCompletionHelper.suggest_custom_suggestions(
						list_to_suggestions(ease_options),
						TextNode,
						TextNode.syntax_highlighter.normal_color
					)
...
func _get_start_code_completion(_CodeCompletionHelper: Node, TextNode: TextEdit) -> void:
	TextNode.add_code_completion_option(
		...
		"join ",
		event_color,
		load("res://addons/dialogic/Editor/Images/Dropdown/join.svg")
	)
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT,
		"leave",
		"leave ",
		event_color,
		load("res://addons/dialogic/Editor/Images/Dropdown/leave.svg")
	)
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT,
		"update",
		"update ",
		event_color,
		load("res://addons/dialogic/Editor/Images/Dropdown/update.svg")
	)
...
func _get_syntax_highlighting(
	...
	if result.get_string("portrait"):
		dict[result.get_start("portrait")] = {
			"color": event_color.lerp(Highlighter.normal_color, 0.6)
		}
		dict[result.get_end("portrait")] = {"color": Highlighter.normal_color}
	if result.get_string("shortcode"):
		dict = Highlighter.color_shortcode_content(
			dict, line, result.get_start("shortcode"), result.get_end("shortcode"), event_color
		)
	return dict
