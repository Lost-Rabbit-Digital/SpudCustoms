...
func _execute() -> void:
	...
	if object:
		while obj_path:
			if (
				obj_path.get_slice(".", 0) in object
				and object.get(obj_path.get_slice(".", 0)) is Object
			):
...
func from_text(string: String) -> void:
	var result := (
		RegEx
		. create_from_string(r"do (?<autoload>[^\(]*)\.((?<method>[^.(]*)(\((?<arguments>.*)\))?)?")
		. search(string.strip_edges())
	)
	if result:
...
func build_event_editor() -> void:
	...
	add_body_edit(
		...
		ValueType.ARRAY,
		{"left_text": "Arguments:"},
		"not autoload_name.is_empty() and not method.is_empty()"
	)
...
func update_argument_info() -> void:
	if (
		autoload_name
		and method
		and not _current_method_arg_hints.is_empty()
		and (_current_method_arg_hints.a == autoload_name and _current_method_arg_hints.m == method)
	):
...
func check_arguments_and_update_warning() -> void:
			...
			if _current_method_arg_hints.info.args[idx].type != typeof(arg):
				...
				ui_update_warning.emit(
					(
						...
						+ str(idx + 1)
						+ " ("
						+ _current_method_arg_hints.info.args[idx].name
						+ ") has the wrong type (method expects "
						+ expected_type
						+ ")!"
					)
				)
	...
	if (
		len(arguments)
		< (
			len(_current_method_arg_hints.info.args)
			- len(_current_method_arg_hints.info.default_args)
		)
	):
		ui_update_warning.emit(
			(
		...
		ui_update_warning.emit(
			(
				"The method is expecting at most "
				+ str(len(_current_method_arg_hints.info.args))
