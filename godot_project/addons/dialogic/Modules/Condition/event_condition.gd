...
func _get_end_branch_control() -> Control:
	return (
		load(get_script().resource_path.get_base_dir().path_join("ui_condition_end.tscn"))
		. instantiate()
	)
...
func is_valid_event(string: String) -> bool:
	if (
		string.strip_edges() in ["if", "elif", "else"]
		or (
			string.strip_edges().begins_with("if ")
			or string.strip_edges().begins_with("elif ")
			or string.strip_edges().begins_with("else")
		)
	):
		return true
	return false
...
func build_event_editor() -> void:
	...
	add_header_edit(
		"condition", ValueType.CONDITION, {}, "condition_type != %s" % ConditionTypes.ELSE
	)
...
func _get_start_code_completion(_CodeCompletionHelper: Node, TextNode: TextEdit) -> void:
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT, "if", "if ", TextNode.syntax_highlighter.code_flow_color
	)
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT, "elif", "elif ", TextNode.syntax_highlighter.code_flow_color
	)
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT, "else", "else:\n	", TextNode.syntax_highlighter.code_flow_color
	)
