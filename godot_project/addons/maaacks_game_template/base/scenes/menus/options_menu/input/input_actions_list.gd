...
func _update_assigned_inputs_and_button(
	action_name: String, action_group: int, input_event: InputEvent
):
...
func _add_action_options(
	action_name: String, readable_action_name: String, input_events: Array[InputEvent]
):
	var new_action_box = %ActionBoxContainer.duplicate()
	new_action_box.visible = true
	new_action_box.vertical = !(vertical)
...
func _assign_input_event_to_action_group(
	input_event: InputEvent, action_name: String, action_group: int
) -> void:
...
func _build_assigned_input_events():
	...
	var action_names := _get_all_action_names(
		show_built_in_actions and catch_built_in_duplicate_inputs
	)
	for action_name in action_names:
		var input_events = InputMap.action_get_events(action_name)
...
func add_action_event(last_input_text: String, last_input_event: InputEvent):
	...
	if last_input_event != null:
		...
		if not assigned_action.is_empty():
			...
			_assign_input_event_to_action_group(
				last_input_event, editing_action_name, editing_action_group
			)
	editing_action_name = ""
