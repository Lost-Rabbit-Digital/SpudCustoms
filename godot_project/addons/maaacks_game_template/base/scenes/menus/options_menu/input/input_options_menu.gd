...
func _open_key_assignment_dialog(
	action_name: String, readable_input_name: String = assignment_placeholder_text
):
...
func _on_input_actions_tree_remove_button_clicked(action_name, input_name):
	...
	$KeyDeletionDialog.dialog_text = tr(KEY_DELETION_TEXT).format(
		{key = input_name, action = action_name}
	)
	$KeyDeletionDialog.popup_centered()


func _popup_already_assigned(action_name, input_name):
	$AlreadyAssignedDialog.dialog_text = tr(ALREADY_ASSIGNED_TEXT).format(
		{key = input_name, action = action_name}
	)
