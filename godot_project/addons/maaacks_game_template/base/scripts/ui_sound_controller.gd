...
func _build_line_stream_players():
	...
	line_text_change_rejected_player = _build_stream_player(
		line_text_change_rejected, "LineTextChangeRejected"
	)
...
func _connect_stream_player(
	node: Node, stream_player: AudioStreamPlayer, signal_name: StringName, callable: Callable
) -> void:
...
func connect_ui_sounds(node: Node) -> void:
	if node is Button:
		...
		_connect_stream_player(
			node, slider_drag_ended_player, &"drag_ended", _slider_drag_ended_play_stream
		)
		...
		_connect_stream_player(
			node, line_text_changed_player, &"text_changed", _line_event_play_stream
		)
		_connect_stream_player(
			node, line_text_submitted_player, &"text_submitted", _line_event_play_stream
		)
		_connect_stream_player(
			node, line_text_change_rejected_player, &"text_change_rejected", _line_event_play_stream
		)
