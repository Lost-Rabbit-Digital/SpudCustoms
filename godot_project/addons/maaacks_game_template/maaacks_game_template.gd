...
func _open_main_scene_confirmation_dialog(current_main_scene: String, new_main_scene: String):
	...
	main_confirmation_instance.dialog_text += (
		MAIN_SCENE_UPDATE_TEXT % [current_main_scene, new_main_scene]
	)
...
func _save_resource(
	...
	whitelisted_extensions: PackedStringArray = []
) -> Error:
	...
	if file_object is Resource:
		...
		if possible_extensions.has(extension):
			return ResourceSaver.save(
				file_object, resource_destination, ResourceSaver.FLAG_CHANGE_PATH
			)
		else:
...
func _add_translations():
	...
	for available_translation in AVAILABLE_TRANSLATIONS:
		var translation_path = (
			get_plugin_path()
			+ ("base/translations/menus_translations.%s.translation" % available_translation)
		)
...
func _open_path_dialog():
	var destination_scene: PackedScene = load(
		get_plugin_path() + "installer/destination_dialog.tscn"
	)
	...
	destination_instance.canceled.connect(
		_check_main_scene_needs_updating.bind(get_plugin_examples_path())
	)
...
func _open_confirmation_dialog():
	...
	confirmation_instance.canceled.connect(
		_check_main_scene_needs_updating.bind(get_plugin_examples_path())
	)
...
func _add_copy_tool_if_examples_exists():
	...
	if dir.dir_exists(examples_path):
		...
		add_tool_menu_item(
			"Delete " + _get_plugin_name() + " Examples...",
			_open_delete_examples_short_confirmation_dialog
		)
...
func _enter_tree():
	...
	add_autoload_singleton(
		"SceneLoader", get_plugin_path() + "base/scenes/autoloads/scene_loader.tscn"
	)
	add_autoload_singleton(
		"ProjectMusicController",
		get_plugin_path() + "base/scenes/autoloads/project_music_controller.tscn"
	)
	add_autoload_singleton(
		"ProjectUISoundController",
		get_plugin_path() + "base/scenes/autoloads/project_ui_sound_controller.tscn"
	)
