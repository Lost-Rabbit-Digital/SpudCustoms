...
func _draw():
	...
	for crater in active_craters:
		...
		for i in range(crater.debris_positions.size()):
			...
			draw_set_transform(
				debris_pos,
				crater.debris_rotations[i],
				Vector2(debris_scale, debris_scale)
			)
			draw_texture(
				debris_texture,
				-debris_texture.get_size() / 2,
				modulate
			)
		...
		if show_debug:
			...
			# Draw black border
			draw_line(
				draw_pos + Vector2(-12, 0),
				draw_pos + Vector2(12, 0),
				border_color,
				border_width
			)
			draw_line(
				draw_pos + Vector2(0, -12),
				draw_pos + Vector2(0, 12),
				border_color,
				border_width
			)
			...
			draw_line(
				draw_pos + Vector2(-10, 0),
				draw_pos + Vector2(10, 0),
				debug_color,
				2.0
			)
			draw_line(
				draw_pos + Vector2(0, -10),
				draw_pos + Vector2(0, 10),
				debug_color,
				2.0
			)
