...
@export
var box_modulate_custom_color: Color = Color(0.47247135639191, 0.31728461384773, 0.16592600941658)
...
func _apply_export_overrides() -> void:
	...
	if !custom_bold_italic_font.is_empty():
		dialog_text.add_theme_font_override(
			&"bold_italics_font", load(custom_bold_italic_font) as Font
		)
	...
	if name_label_use_global_size:
		name_label.add_theme_font_size_override(
			&"font_size", get_global_setting(&"font_size", name_label_custom_size) as int
		)
	else:
	...
	if (
		name_label_use_global_font
		and ResourceLoader.exists(get_global_setting(&"font", "") as String)
	):
