...
enum LeaderboardState { IDLE, FINDING, UPLOADING, DOWNLOADING, COMPLETED, ERROR }
...
func _ready():
	...
	_connect_signal(
		Steam.leaderboard_find_result, _on_leaderboard_find_result, "leaderboard_find_result"
	)
	_connect_signal(
		Steam.leaderboard_score_uploaded,
		_on_leaderboard_score_uploaded,
		"leaderboard_score_uploaded"
	)
	_connect_signal(
		Steam.leaderboard_scores_downloaded,
		_on_leaderboard_scores_downloaded,
...
func _check_app_ownership():
	...
	if OS.has_feature("editor") || OS.has_feature("debug"):
		LogManager.write_info(
			"Development mode detected - enabling Steam features regardless of ownership"
		)
		owns_app = true
...
func _process(delta: float) -> void:
	...
	for i in range(pending_operations.size() - 1, -1, -1):
		...
		if op.time_elapsed > 10.0:
			...
			if (
				op.name.begins_with("leaderboard_")
...
func _log_current_state():
	if is_fetching_leaderboard:
		LogManager.write_steam(
			"Leaderboard state: " + LeaderboardState.keys()[current_leaderboard_state]
		)
...
func submit_score(score: int, difficulty: String = "Normal", shift: int = -1) -> bool:
	...
	pending_operations.append(
		{"name": "leaderboard_upload_" + leaderboard_name, "score": score, "time_elapsed": 0.0}
	)
...
func request_leaderboard_entries(difficulty: String = "Normal", shift: int = -1) -> bool:
	...
	# Verify Steam is running and user is logged in
	if not _verify_steam_connection():
		LogManager.write_error(
			"Steam not running or user not logged in, leaderboard request skipped"
		)
		emit_signal("leaderboard_updated", [])
		return false
	...
	if is_fetching_leaderboard:
		LogManager.write_warning(
			(
				"Leaderboard operation already in progress (state: "
				+ LeaderboardState.keys()[current_leaderboard_state]
				+ "), cannot request entries"
			)
		)
	...
	# Add to pending operations
	pending_operations.append(
		{"name": "leaderboard_download_" + leaderboard_name, "time_elapsed": 0.0}
	)
...
func _download_leaderboard_entries() -> bool:
	if (
		...
		LogManager.write_error(
			(
				"Cannot download entries - wrong state: "
				+ str(LeaderboardState.keys()[current_leaderboard_state])
			)
		)
		return false
	...
	if current_leaderboard_handle <= 0:
		LogManager.write_error(
			(
				"Cannot download leaderboard entries - invalid handle: "
				+ str(current_leaderboard_handle)
			)
		)
		current_leaderboard_state = LeaderboardState.ERROR
		is_fetching_leaderboard = false
	...
	# Track API call
	api_call_attempts["downloadLeaderboardEntries"] = (
		api_call_attempts.get("downloadLeaderboardEntries", 0) + 1
	)

	# Request global entries - using void call properly
	Steam.downloadLeaderboardEntries(
		1, 12, Steam.LEADERBOARD_DATA_REQUEST_GLOBAL, current_leaderboard_handle  # Start rank  # End rank
	)
...
func _on_leaderboard_find_result(handle: int, found: bool) -> void:
	LogManager.write_steam(
		"Leaderboard find result - Handle: " + str(handle) + ", Found: " + str(found)
	)

	# Track API result
	api_call_results["findLeaderboard"] = {
		"handle": handle, "found": found, "timestamp": Time.get_unix_time_from_system()
	}
	...
	if current_leaderboard_state != LeaderboardState.FINDING:
		LogManager.write_warning(
			(
				"Unexpected leaderboard_find_result callback in state: "
				+ LeaderboardState.keys()[current_leaderboard_state]
	...
	for op in pending_operations:
		if op.name.begins_with("leaderboard_upload_"):
			...
			# Track API call
			api_call_attempts["uploadLeaderboardScore"] = (
				api_call_attempts.get("uploadLeaderboardScore", 0) + 1
			)
...
func _on_leaderboard_score_uploaded(success: int, handle: int, score_details: Dictionary) -> void:
	LogManager.write_steam(
		"Score upload result - Success: " + str(success) + ", Handle: " + str(handle)
	)
	...
	if current_leaderboard_state != LeaderboardState.UPLOADING:
		LogManager.write_warning(
			(
				"Unexpected leaderboard_score_uploaded callback in state: "
				+ LeaderboardState.keys()[current_leaderboard_state]
			)
		)
		return
...
func _on_leaderboard_scores_downloaded(
	message: String, this_leaderboard_handle: int, result: Array
) -> void:
	LogManager.write_steam("Scores downloaded message: " + message)
	LogManager.write_steam(
		"Leaderboard handle: " + str(this_leaderboard_handle) + ", Entries: " + str(result.size())
	)
	...
	if current_leaderboard_state != LeaderboardState.DOWNLOADING:
		LogManager.write_warning(
			(
				"Unexpected leaderboard_scores_downloaded callback in state: "
				+ LeaderboardState.keys()[current_leaderboard_state]
			)
		)
	...
	LogManager.write_steam(
		"Updated cached leaderboard entries: " + str(cached_leaderboard_entries.size())
	)
	emit_signal("leaderboard_updated", cached_leaderboard_entries)
...
func check_achievements(
	...
	total_runners_stopped: int,
	perfect_hits: int,
	score: int,
	current_story_state: int
):
	if not _verify_steam_connection():
		return
	...
	LogManager.write_steam(
		(
			"Checking achievements - Shifts: "
			+ str(total_shifts_completed)
			+ ", Story: "
			+ str(current_story_state)
		)
	)
...
func update_steam_stats(
	total_shifts_completed: int, total_runners_stopped: int, perfect_hits: int, score: int
):
	if not _verify_steam_connection():
		return
