...
func test_get_missile_bonus_with_one_perfect_hit():
	...
	assert_eq(shift_stats.get_missile_bonus(), 150, "Missile bonus should be 150 with 1 perfect hit")

func test_get_missile_bonus_with_multiple_perfect_hits():
	shift_stats.perfect_hits = 5
	assert_eq(shift_stats.get_missile_bonus(), 750, "Missile bonus should be 750 with 5 perfect hits (5 * 150)")
...
func test_get_accuracy_bonus_with_perfect_stamps():
	...
	assert_eq(shift_stats.get_accuracy_bonus(), 600, "Accuracy bonus should be 600 with 3 perfect stamps (3 * 200)")

func test_get_accuracy_bonus_all_perfect():
	shift_stats.total_stamps = 10
	shift_stats.perfect_stamps = 10
	assert_eq(shift_stats.get_accuracy_bonus(), 2000, "Accuracy bonus should be 2000 with 10 perfect stamps")
...
func test_get_speed_bonus_with_time_remaining():
	...
	assert_eq(shift_stats.get_speed_bonus(), 500, "Speed bonus should be 500 with 5 seconds remaining (5 * 100)")

func test_get_speed_bonus_with_partial_second():
	shift_stats.processing_time_left = 3.7
	assert_eq(shift_stats.get_speed_bonus(), 370, "Speed bonus should be 370 with 3.7 seconds remaining")
...
func test_get_total_bonus_combines_all_bonuses():
	...
	assert_eq(shift_stats.get_total_bonus(), expected_total, "Total bonus should sum all bonuses: 1300")
...
func test_missile_bonus_with_large_number():
	...
	assert_eq(shift_stats.get_missile_bonus(), 150000, "Missile bonus should handle large numbers correctly")
