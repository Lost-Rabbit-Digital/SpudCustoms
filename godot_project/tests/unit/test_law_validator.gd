...
func test_calculate_age_exact_birthday():
	...
	var birth_date = (
		"%04d.%02d.%02d" % [current_date.year - 5, current_date.month, current_date.day]
	)
...
func test_is_expired_past_date():
	var current_date = Time.get_date_dict_from_system()
	var expired_date = (
		"%04d.%02d.%02d" % [current_date.year - 1, current_date.month, current_date.day]
	)
...
func test_check_violations_fresh_potatoes_rule():
	...
	assert_string_contains(
		result.violation_reason, "not_fresh", "Violation reason should mention not fresh"
	)
...
func test_check_violations_expired_document():
	...
	assert_string_contains(
		result.violation_reason, "expired", "Violation should mention expiration"
	)
