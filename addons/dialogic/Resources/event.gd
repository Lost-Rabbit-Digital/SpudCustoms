...
func get_property_translated(property_name: String) -> String:
	if (
		...
		return (
			translation
			if translation != get_property_translation_key(property_name)
			else _get_property_original_translation(property_name)
		)
	else:
...
func value_to_string(value: Variant, suggestions := Callable()) -> String:
	...
	if not (
		(value_as_string.begins_with("[") and value_as_string.ends_with("]"))
		or (value_as_string.begins_with("{") and value_as_string.ends_with("}"))
	):
...
func is_valid_event(string: String) -> bool:
	if (
		string.strip_edges().begins_with("[" + get_shortcode() + " ")
		or string.strip_edges().begins_with("[" + get_shortcode() + "]")
	):
		return true
	return false
...
func _get_code_completion(
	_CodeCompletionHelper: Node, _TextNode: TextEdit, _line: String, _word: String, _symbol: String
) -> void:
	pass
...
func _get_syntax_highlighting(
	_Highlighter: SyntaxHighlighter, dict: Dictionary, _line: String
) -> Dictionary:
	return dict
...
func add_header_edit(
	variable: String, editor_type := ValueType.LABEL, extra_info := {}, condition := ""
) -> void:
	(
		editor_list
		. append(
...
func add_header_button(
	text: String, callable: Callable, tooltip: String, icon: Variant = null, condition := ""
) -> void:
	(
		editor_list
		. append(
			{
				...
				"display_info":
				{"text": text, "tooltip": tooltip, "callable": callable, "icon": icon},
				"condition": condition,
			}
		)
	)
