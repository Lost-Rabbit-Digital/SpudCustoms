...
func _ready() -> void:
	...
	%RightSidebar.size.x = DialogicUtil.get_editor_setting(
		"dialogic/editor/right_sidebar_width", 200 * editor_scale
	)
	$View.split_offset = -DialogicUtil.get_editor_setting(
		"dialogic/editor/right_sidebar_width", 200 * editor_scale
	)
	sidebar_collapsed = DialogicUtil.get_editor_setting(
		"dialogic/editor/right_sidebar_collapsed", false
	)
...
func load_event_buttons() -> void:
	...
	var button_scene := load(
		"res://addons/dialogic/Editor/TimelineEditor/VisualEditor/AddEventButton.tscn"
	)
	...
	for event_script in scripts:
		...
		# Sort event button
		while (
			event_resource.event_sorting_index
			< (
				sections[event_resource.event_category]
				. get_child(max(0, button.get_index() - 1))
				. resource
				. event_sorting_index
			)
		):
	...
	# Sort event sections
	var sections_order: Array = DialogicUtil.get_editor_setting(
		"event_section_order", ["Main", "Flow", "Logic", "Audio", "Visual", "Other", "Helper"]
	)

	sections_order.reverse()
	for section_name in sections_order:
		if %RightSidebar.get_child(0).has_node(section_name):
			%RightSidebar.get_child(0).move_child(
				%RightSidebar.get_child(0).get_node(section_name), 0
			)
	...
	if len(selected_items) > 0 and event is InputEventMouseMotion:
		if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
			if !%TimelineArea.dragging and !get_viewport().gui_is_dragging() and drag_allowed:
				sort_selection()
				%TimelineArea.start_dragging(
					%TimelineArea.DragTypes.EXISTING_EVENTS, selected_items
				)
...
func create_end_branch_event(at_index: int, parent_node: Node) -> Node:
	var end_branch_event: Control = (
		load("res://addons/dialogic/Editor/Events/BranchEnd.tscn").instantiate()
	)
...
func add_event_with_end_branch(
	resource, at_index: int = -1, auto_select: bool = false, indent: bool = false
) -> void:
...
func add_event_undoable(event_resource: DialogicEvent, at_index: int = -1) -> void:
	...
	if event_resource.can_contain_events:
		TimelineUndoRedo.add_do_method(
			add_event_with_end_branch.bind(event_resource, at_index, true, true)
		)
			...
			if idx.begins_with("#"):  # a global index
				events.append(
					create_end_branch_event(
						%Timeline.get_child_count(), %Timeline.get_child(int(idx.trim_prefix("#")))
					)
				)
			else:  # a local index (index in the added events list)
				events.append(
					create_end_branch_event(%Timeline.get_child_count(), events[int(idx)])
				)
...
func delete_events_indexed(indexed_events: Dictionary) -> void:
	...
	for idx in indexed_events:
		if (
			"end_node" in %Timeline.get_child(idx - idx_shift)
			and %Timeline.get_child(idx - idx_shift).end_node != null
			and is_instance_valid(%Timeline.get_child(idx - idx_shift).end_node)
		):
			%Timeline.get_child(idx - idx_shift).end_node.parent_node = null
		if (
			%Timeline.get_child(idx - idx_shift) != null
			and is_instance_valid(%Timeline.get_child(idx - idx_shift))
		):
			...
			%Timeline.get_child(idx - idx_shift).get_parent().remove_child(
				%Timeline.get_child(idx - idx_shift)
			)
			idx_shift += 1
...
func delete_selected_events() -> void:
	...
	var next_node := %Timeline.get_child(
		mini(%Timeline.get_child_count() - 1, selected_items[-1].get_index() + 1)
	)
...
func move_blocks_to_index(blocks: Array, index: int):
	...
	for event in blocks:
		...
		undo_indexes[index - index_shift + event_count] = (
			event.get_index() + index_shift * int(index < event.get_index())
		)  #+int((index -index_shift+event_count) < event.get_index())
		event_count += 1
	...
	for idx in do_indexes:
		...
		for c_idx in range(check_from, check_to):
			...
			if (
				c_event.resource is DialogicEndBranchEvent
				and c_event.parent_node.get_index() < check_from
			):
				return
...
func move_events_by_indexes(index_dict: Dictionary) -> void:
	...
	for idx in sorted_indexes:
		...
		if idx < index_dict[idx]:
			index_dict[idx] -= len(
				sorted_indexes.filter(func(x): return x <= index_dict[idx] - count - 1)
			)
...
func scroll_to_piece(piece_index: int) -> void:
	await get_tree().process_frame
	var height: float = (
		%Timeline.get_child(min(piece_index, %Timeline.get_child_count() - 1)).position.y
	)
	if (
		height < %TimelineArea.scroll_vertical
		or height > %TimelineArea.scroll_vertical + %TimelineArea.size.y
	):
...
func indent_events() -> void:
	...
	for block in event_list:
		...
		if (
			(not currently_hidden)
			and block.resource.can_contain_events
			and block.end_node
			and block.collapsed
		):
...
func _on_event_popup_menu_id_pressed(id: int) -> void:
	...
	if id == 0:
		...
		find_parent("EditorView").plugin_reference.get_editor_interface().set_main_screen_editor(
			"Script"
		)
		find_parent("EditorView").plugin_reference.get_editor_interface().edit_script(
			item.resource.get_script(), 1, 1
		)
	elif id == 4 or id == 5:
		if id == 4:
...
func _on_right_sidebar_resized() -> void:
	...
	if _initialized:
		...
		DialogicUtil.set_editor_setting(
			"dialogic/editor/right_sidebar_collapsed", sidebar_collapsed
		)
...
func _input(event: InputEvent) -> void:
	...
	match event.as_text():
		...
		"Ctrl+Shift+T", "Ctrl+Alt+T", "Ctrl+Option+T":  # Add text event with current or previous character
			...
			ev.character = get_previous_character(
				event.as_text() == "Ctrl+Alt+T" or event.as_text() == "Ctrl+Option+T"
			)
	...
	if (
		...
		or (focus_owner is Button and focus_owner.get_parent_control().name == "Spin")
	):
		return
	...
	match event.as_text():
		...
		"Ctrl+V":
			...
			if events_list:
				TimelineUndoRedo.create_action(
					"[D] Pasting " + str(len(events_list)) + " event(s)."
				)
				TimelineUndoRedo.add_do_method(
					add_events_at_index.bind(events_list, paste_position)
				)
				TimelineUndoRedo.add_undo_method(
					delete_events_at_index.bind(paste_position, len(events_list))
				)
				TimelineUndoRedo.commit_action()
				get_viewport().set_input_as_handled()
...
func get_previous_character(double_previous := false) -> DialogicCharacter:
	...
	for i in range(selected_items[0].get_index() + 1):
		idx -= 1
		if !(
			"resource" in %Timeline.get_child(idx)
			and "character" in %Timeline.get_child(idx).resource
		):
			continue
...
func search_navigate(navigate_up := false) -> void:
	...
	if next_pos:
		...
		field.call_deferred(
			"select", result.y, result.x, result.y, result.x + len(get_meta("current_search"))
		)
...
func get_next_search_position(navigate_up := false, include_current := false) -> Array:
	...
	if (
		get_viewport().gui_get_focus_owner() is TextEdit
		and get_viewport().gui_get_focus_owner() is DialogicVisualEditorField
	):
	...
	while true:
		...
		var result := search_text_field(
			field, search_text, search_flags, navigate_up, first and include_current
		)
		...
		event = search_results.keys()[wrapi(
			index + (-1 if navigate_up else 1), 0, search_results.size()
		)]
...
func search_text_field(
	field: TextEdit, search_text := "", flags := 0, navigate_up := false, include_current := false
) -> Vector2i:
