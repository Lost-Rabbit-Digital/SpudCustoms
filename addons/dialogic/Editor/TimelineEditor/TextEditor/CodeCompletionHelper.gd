...
var syntax_highlighter: SyntaxHighlighter = (
	load("res://addons/dialogic/Editor/TimelineEditor/TextEditor/syntax_highlighter.gd").new()
)
var text_syntax_highlighter: SyntaxHighlighter = (
	load("res://addons/dialogic/Editor/TimelineEditor/TextEditor/syntax_highlighter.gd").new()
)
...
func _ready() -> void:
	...
	completion_shortcode_param_getter_regex.compile(
		'(?<param>\\w*)\\W*=\\s*"?(\\w|\\s)*' + String.chr(0xFFFF)
	)
	completion_shortcode_value_regex.compile(
		r'(\[|\s)[^\[\s=]*="(?<value>[^"$]*)' + String.chr(0xFFFF)
	)
...
func get_code_completion_line(text: CodeEdit) -> String:
	return (
		text
		. get_line(text.get_caret_line())
		. insert(text.get_caret_column(), String.chr(0xFFFF))
		. strip_edges()
	)
...
func request_code_completion(force: bool, text: CodeEdit, mode := Modes.FULL_HIGHLIGHTING) -> void:
	...
	if (
		mode == Modes.FULL_HIGHLIGHTING
		and syntax_highlighter.line_is_shortcode_event(text.get_caret_line())
	):
		if symbol == "[":
			...
			for shortcode in shortcodes:
				...
				if shortcode_events[shortcode].get_shortcode_parameters().is_empty():
					text.add_code_completion_option(
						CodeEdit.KIND_MEMBER,
						shortcode,
						shortcode,
						shortcode_events[shortcode].event_color.lerp(
							syntax_highlighter.normal_color, 0.3
						),
						shortcode_events[shortcode]._get_icon()
					)
				else:
					text.add_code_completion_option(
						CodeEdit.KIND_MEMBER,
				...
				for param in parameters:
					if !param + "=" in full_event_text:
						text.add_code_completion_option(
							...
							param,
							param + '="',
							shortcode_events[code].event_color.lerp(
								syntax_highlighter.normal_color, 0.3
							),
							text.get_theme_icon("MemberProperty", "EditorIcons")
						)
...
func suggest_characters(
	text: CodeEdit, type := CodeEdit.KIND_MEMBER, event: DialogicEvent = null
) -> void:
	for character in DialogicResourceUtil.get_character_directory():
		...
		if (
			...
			and load(DialogicResourceUtil.get_character_directory()[character]).portraits.is_empty()
		):
			result += ": "
		...
		text.add_code_completion_option(
			...
			character,
			result,
			syntax_highlighter.character_name_color,
...
func suggest_timelines(text: CodeEdit, type := CodeEdit.KIND_MEMBER, color := Color()) -> void:
	for timeline in DialogicResourceUtil.get_timeline_directory():
		text.add_code_completion_option(
			type, timeline, timeline + "/", color, text.get_theme_icon("TripleBar", "EditorIcons")
		)
	...
	if timeline in DialogicResourceUtil.get_label_cache():
		for i in DialogicResourceUtil.get_label_cache()[timeline]:
			text.add_code_completion_option(
				...
				i + end,
				color,
				load("res://addons/dialogic/Modules/Jump/icon_label.png")
			)
...
func suggest_portraits(text: CodeEdit, character_name: String, end_check := ")") -> void:
	if not character_name in DialogicResourceUtil.get_character_directory():
		return
	var character_resource: DialogicCharacter = load(
		DialogicResourceUtil.get_character_directory()[character_name]
	)
	...
	if character_resource.portraits.is_empty():
		text.add_code_completion_option(
			...
			"",
			syntax_highlighter.character_portrait_color,
			load("res://addons/dialogic/Editor/Images/Pieces/warning.svg")
		)
...
func suggest_variables(text: CodeEdit):
	for variable in DialogicUtil.list_variables(ProjectSettings.get_setting("dialogic/variables")):
		text.add_code_completion_option(
			...
			variable,
			variable,
			syntax_highlighter.variable_color,
			text.get_theme_icon("MemberProperty", "EditorIcons"),
			"}"
		)
...
func suggest_bool(text: CodeEdit, color: Color):
	text.add_code_completion_option(
		...
		"true",
		color,
		text.get_theme_icon("GuiChecked", "EditorIcons"),
		'" '
	)
	text.add_code_completion_option(
		CodeEdit.KIND_VARIABLE,
		...
		"false",
		color,
		text.get_theme_icon("GuiUnchecked", "EditorIcons"),
		'" '
	)
	...
	for key in suggestions.keys():
		if suggestions[key].has("text_alt"):
			text.add_code_completion_option(
				CodeEdit.KIND_VARIABLE,
				key,
				suggestions[key].text_alt[0],
				color,
				suggestions[key].get("icon", null),
				'" '
			)
		else:
			text.add_code_completion_option(
...
func suggest_shortcode_values(
	text: CodeEdit, event: DialogicEvent, line: String, word: String
) -> void:
	...
	if !event.get_shortcode_parameters()[current_parameter].has("suggestions"):
		if typeof(event.get_shortcode_parameters()[current_parameter].default) == TYPE_BOOL:
			...
			text.add_code_completion_option(
				...
				word,
				word,
				event.event_color.lerp(syntax_highlighter.normal_color, 0.3),
				text.get_theme_icon("GuiScrollArrowRight", "EditorIcons"),
				'" '
			)
		return

	var suggestions: Dictionary = (
		event.get_shortcode_parameters()[current_parameter]["suggestions"].call()
	)
	suggest_custom_suggestions(
		suggestions, text, event.event_color.lerp(syntax_highlighter.normal_color, 0.3)
	)
	...
	for candidate in candidates:
		if candidate.kind == text.KIND_PLAIN_TEXT:
			...
			if (
				...
				or current_word.to_lower() in candidate.insert_text.to_lower()
			):
				valid_candidates.append(candidate)
		elif candidate.kind == text.KIND_VARIABLE:
			var current_param_value := get_code_completion_parameter_value(text)
			if (
				current_param_value.is_empty()
				or current_param_value.to_lower() in candidate.insert_text.to_lower()
			):
				valid_candidates.append(candidate)
			...
			if (
				!current_word.is_empty()
				and current_word.to_lower() in candidate.insert_text.to_lower()
			):
				valid_candidates.append(candidate)
...
func confirm_code_completion(replace: bool, text: CodeEdit) -> void:
	...
	var code_completion := text.get_code_completion_option(
		text.get_code_completion_selected_index()
	)
	...
	text.remove_text(
		text.get_caret_line(),
		text.get_caret_column() - len(word),
		text.get_caret_line(),
		text.get_caret_column()
	)
	...
	if (
		code_completion.has("default_value")
		and typeof(code_completion["default_value"]) == TYPE_STRING
	):
		var next_letter := text.get_line(text.get_caret_line()).substr(
			text.get_caret_column(), len(code_completion["default_value"])
		)
		if (
			next_letter
			and (
				next_letter == code_completion["default_value"]
				or next_letter[0] == code_completion["default_value"][0]
			)
		):
...
func symbol_lookup(symbol: String, line: int, column: int) -> void:
	...
	if symbol in DialogicResourceUtil.get_character_directory():
		EditorInterface.edit_resource(
			DialogicResourceUtil.get_resource_from_identifier(symbol, "dch")
		)
	if symbol in DialogicResourceUtil.get_timeline_directory():
		EditorInterface.edit_resource(
			DialogicResourceUtil.get_resource_from_identifier(symbol, "dtl")
		)
