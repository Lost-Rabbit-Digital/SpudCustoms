...
var shortcode_param_regex := RegEx.create_from_string(
...
func update_colors() -> void:
	...
	var editor_settings: EditorSettings = (
		DialogicUtil.get_dialogic_plugin().get_editor_interface().get_editor_settings()
	)
	...
	code_flow_color = editor_settings.get(
	...
	character_name_color = editor_settings.get("text_editor/theme/highlighting/symbol_color").lerp(
		normal_color, 0.3
	)
			...
			if result:
				if result.get_string("id") in shortcode_events:
					if full_event.begins_with(str_line):
						dict[result.get_start("id")] = {
							"color":
							shortcode_events[result.get_string("id")].event_color.lerp(
								normal_color, 0.4
							)
						}
						...
						if result.get_string("args"):
							color_shortcode_content(
								dict,
								str_line,
								result.get_start("args"),
								result.get_end("args"),
								shortcode_events[result.get_string("id")].event_color
							)
					else:
						color_shortcode_content(
							dict,
...
func color_word(
	dict: Dictionary, color: Color, line: String, word: String, from: int = 0, to: int = 0
) -> Dictionary:
...
func color_shortcode_content(
	dict: Dictionary, line: String, from: int = 0, to: int = 0, base_color := normal_color
) -> Dictionary:
	if to <= from:
		to = len(line) - 1
