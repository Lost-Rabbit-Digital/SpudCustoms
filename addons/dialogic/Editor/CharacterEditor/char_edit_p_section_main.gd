...
func _on_open_scene_button_pressed() -> void:
	...
	if ResourceLoader.exists(data.get("scene", "")):
		DialogicUtil.get_dialogic_plugin().get_editor_interface().open_scene_from_path(
			data.get("scene", "")
		)
	...
	match part_info.type:
		...
		"Preset":
			find_parent("EditorView").godot_file_dialog(
				...
				(
					part_info.path.get_file().trim_suffix("." + part_info.path.get_extension())
					+ "_"
					+ character_editor.current_resource.get_character_name().to_lower()
				)
			)
		"Custom":
			(
...
func make_portrait_preset_custom(target_file: String, info: Dictionary) -> String:
	...
	var result_path := DialogicUtil.make_file_custom(
		previous_file, target_file.get_base_dir(), target_file.get_file()
	)
...
func reload_ui(data: Dictionary) -> void:
	...
	if path.is_empty():
		...
		%SceneLabel.add_theme_color_override(
		...
		%SceneLabel.add_theme_color_override(
			"font_color", get_theme_color("accent_color", "Editor")
		)
		...
		%SceneLabel.tooltip_text = path
		%SceneLabel.add_theme_color_override(
			"font_color", get_theme_color("property_color_x", "Editor")
		)
