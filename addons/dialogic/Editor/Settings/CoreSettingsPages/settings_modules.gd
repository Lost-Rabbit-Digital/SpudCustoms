...
func _ready() -> void:
	...
	%EventDefaultsPanel.add_theme_stylebox_override(
	...
	for child in %Tree.get_root().get_children():
		if new_text.to_lower() in child.get_text(0).to_lower() or new_text.is_empty():
			for sub_child in child.get_children():
				...
				sub_child.get_meta("filter_button").set_meta(
					"counter", sub_child.get_meta("filter_button").get_meta("counter") + 1
				)
				sub_child.get_meta("filter_button").text = str(
					sub_child.get_meta("filter_button").get_meta("counter")
				)
			child.visible = true
		else:
			for sub_child in child.get_children():
				sub_child.visible = (
					sub_child.get_meta("filter_button").button_pressed
					and new_text.to_lower() in sub_child.get_text(0).to_lower()
				)
		...
		for sub_child in child.get_children():
			if sub_child.visible:
				...
				if (
					sub_child.get_metadata(0)
					and sub_child.get_metadata(0)["type"] == "Event"
					and sub_child.get_metadata(0)["hidden"]
				):
...
func load_modules_tree() -> void:
			...
			for cached_event in cached_events:
				if cached_event.get_script().resource_path == ev:
					...
					event_item.set_metadata(
						0, {"type": "Event", "event": cached_event, "hidden": hidden}
					)
					event_item.add_button(
						0,
						get_theme_icon("GuiVisibilityVisible", "EditorIcons"),
						0,
						false,
						"Toggle Event Button Visibility"
					)
					if hidden:
						event_item.set_button(
							0, 0, get_theme_icon("GuiVisibilityHidden", "EditorIcons")
						)
	...
	if %Tree.get_root().get_child_count():
		%Tree.set_selected(%Tree.get_root().get_child(0), 0)
		...
		"Event":
			...
			if id == 0:
				...
				if meta["hidden"]:
					...
					item.get_parent().set_button_color(
						0, item.get_index(), item.get_icon_modulate(0)
					)
		...
		match metadata.type:
			"Event":
				...
				if !metadata.event.disable_editor_button:
					...
					%VisibilityToggle.button_pressed = !(
						metadata.event.event_name
						in DialogicUtil.get_editor_setting("hidden_event_buttons", [])
					)
					if %VisibilityToggle.button_pressed:
						%VisibilityToggle.icon = get_theme_icon(
							"GuiVisibilityVisible", "EditorIcons"
						)
					else:
						%VisibilityToggle.icon = get_theme_icon(
							"GuiVisibilityHidden", "EditorIcons"
						)
			# -------------------------------------------------
			"Subsystem":
...
func _on_visibility_toggle_toggled(button_pressed: bool) -> void:
	...
	if button_pressed:
		...
		%Tree.get_selected().get_parent().set_button_color(
			0, %Tree.get_selected().get_index(), %Tree.get_selected().get_icon_modulate(0)
		)
	else:
		...
		color.a = 0.5
		%Tree.get_selected().get_parent().set_button_color(
			0, %Tree.get_selected().get_index(), color
		)
...
func load_event_settings(event: DialogicEvent) -> void:
	...
	var event_default_overrides: Dictionary = ProjectSettings.get_setting(
		"dialogic/event_default_overrides", {}
	)
	...
	for prop in params:
		...
		if event_default_overrides.get(event.event_name, {}).has(params[prop].property):
			current_value = event_default_overrides.get(event.event_name, {}).get(
		...
		match typeof(event.get(params[prop].property)):
			TYPE_STRING:
				...
				editor_node.text_changed.connect(
					_on_event_default_string_submitted.bind(params[prop].property)
				)
			TYPE_INT, TYPE_FLOAT:
				if params[prop].has("suggestions"):
					...
					editor_node.item_selected.connect(
						_on_event_default_option_selected.bind(editor_node, params[prop].property)
					)
				else:
					...
					editor_node.value_changed.connect(
						_on_event_default_number_changed.bind(params[prop].property)
					)

			TYPE_VECTOR2:
				editor_node = (
					load("res://addons/dialogic/Editor/Events/Fields/field_vector2.tscn")
					. instantiate()
				)
			...
			TYPE_BOOL:
				...
				editor_node.toggled.connect(
					_on_event_default_bool_toggled.bind(params[prop].property)
				)

			TYPE_ARRAY:
				editor_node = (
					load("res://addons/dialogic/Editor/Events/Fields/field_array.tscn")
					. instantiate()
				)
				editor_node.set_value(current_value)
				editor_node.property_name = params[prop].property
		...
		reset.pressed.connect(
			reset_event_default_override.bind(prop, editor_node, params[prop].default)
		)
...
func _on_event_default_option_selected(
	index: int, option_button: OptionButton, prop: String
) -> void:
