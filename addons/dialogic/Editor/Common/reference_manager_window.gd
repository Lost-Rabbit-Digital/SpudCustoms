...
func _ready() -> void:
	...
	icon_button = editors_manager.add_icon_button(
		get_theme_icon("Unlinked", "EditorIcons"), "Reference Manager"
	)
	...
	get_parent().plugin_reference.get_editor_interface().get_file_system_dock().files_moved.connect(
		_on_file_moved
	)
	(
		get_parent()
	...
	match type:
		Types.TEXT:
			...
			if "<replace>" in old_name:
				...
				regexes = [r"(?<replace>%s)" % old_name.replace("/", "\\/")]
				if !case_sensitive:
		...
		Types.TIMELINE_NAME:
			regexes = [r'timeline ?= ?" ?(?<replace>%s) ?"' % old_name]
...
func add_portrait_ref_change(
	old_name: String, new_name: String, character_names: PackedStringArray
) -> void:
