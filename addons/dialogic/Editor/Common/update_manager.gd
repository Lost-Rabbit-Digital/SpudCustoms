...
func request_update_download() -> void:
	...
	if DirAccess.dir_exists_absolute("res://test-project/"):
		prints(
			"[Dialogic] Looks like you are working on the addon. You can't update the addon from within itself."
		)
	...
	for path in files:
		...
		if path.ends_with("/"):
			...
			var file: FileAccess = FileAccess.open(
				"res://addons/".path_join(new_file_path), FileAccess.WRITE
			)
