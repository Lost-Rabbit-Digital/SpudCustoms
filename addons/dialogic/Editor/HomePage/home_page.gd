...
func _ready() -> void:
	...
	%RandomTipLabel.add_theme_color_override(
		"font_color", get_theme_color("property_color_z", "Editor")
	)
...
func _open(extra_info: Variant = "") -> void:
	if tips.is_empty():
		var file := FileAccess.open(
			"res://addons/dialogic/Editor/HomePage/tips.txt", FileAccess.READ
		)
...
func _on_tip_action(action: String) -> void:
	if action.begins_with("https://"):
		...
		if editor_name in editors_manager.editors:
			editors_manager.open_editor(
				editors_manager.editors[editor_name].node, false, extra_info
			)
			return
