...
func _enter_tree() -> void:
	%HiddenLabel.add_theme_color_override(
		"font_color", event_resource.event_color.lerp(get_theme_color("font_color", "Editor"), 0.8)
	)
...
func get_preview_size() -> int:
	return (
		DialogicUtil.get_editor_setting("image_preview_height", 50)
		* DialogicUtil.get_editor_scale()
	)
