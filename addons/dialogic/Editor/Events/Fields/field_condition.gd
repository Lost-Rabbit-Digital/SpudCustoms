...
func _ready() -> void:
	for i in [%Value1Type, %Value2Type]:
		i.options = [
			{"label": "String", "icon": ["String", "EditorIcons"], "value": 0},
			{"label": "Number", "icon": ["float", "EditorIcons"], "value": 1},
			{
				"label": "Variable",
				"icon": load("res://addons/dialogic/Editor/Images/Pieces/variable.svg"),
				"value": 2
			},
			{"label": "Bool", "icon": ["bool", "EditorIcons"], "value": 3},
			{"label": "Expression", "icon": ["Variant", "EditorIcons"], "value": 4}
		]
		i.symbol_only = true
...
func trim_value(value: Variant, value_type: int) -> String:
	...
	match value_type:
		0:
			return value.trim_prefix('"').trim_suffix('"').replace('\\"', '"')
		2:
...
func something_changed(fake_arg1 = null, fake_arg2 = null):
	...
	match %Value1Type.current_value:
		0:
			_current_value1 = prep_value(%Value1Text.text, %Value1Type.current_value)
		1:
			_current_value1 = str(%Value1Number.get_value())
		2:
			_current_value1 = prep_value(%Value1Variable.current_value, %Value1Type.current_value)
		3:
			_current_value1 = prep_value(%Value1Bool.button_pressed, %Value1Type.current_value)
		_:
			_current_value1 = prep_value(%Value1Text.text, %Value1Type.current_value)

	match %Value2Type.current_value:
		0:
			_current_value2 = prep_value(%Value2Text.text, %Value2Type.current_value)
		1:
			_current_value2 = str(%Value2Number.get_value())
		2:
			_current_value2 = prep_value(%Value2Variable.current_value, %Value2Type.current_value)
		3:
			_current_value2 = prep_value(%Value2Bool.button_pressed, %Value2Type.current_value)
		_:
			_current_value2 = prep_value(%Value2Text.text, %Value2Type.current_value)
	...
	if event_resource:
		if not %Operator.text in ["==", "!="] and get_value_type(_current_value2, 0) in [0, 3]:
			event_resource.ui_update_warning.emit(
				"This operator doesn't work with strings and booleans."
			)
		else:
			event_resource.ui_update_warning.emit("")
