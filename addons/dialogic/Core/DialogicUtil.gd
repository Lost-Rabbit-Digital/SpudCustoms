			...
			if not file_name.begins_with("."):
				if files_only:
					if (
						...
						and (not file_name.ends_with(".import") or include_imports)
					):
						if full_file_path:
...
static func get_module_path(name: String, builtin := true) -> String:
	if builtin:
		...
		return (
			ProjectSettings
			. get_setting("dialogic/extensions_folder", "res://addons/dialogic_additions")
			. path_join(name)
		)
...
static func _update_autoload_subsystem_access() -> void:
	...
	script.source_code = (
		RegEx
		. create_from_string(r"#region SUBSYSTEMS\n#*\n((?!#endregion)(.*\n))*#endregion")
		. sub(script.source_code, new_subsystem_access_list)
	)
	ResourceSaver.save(script)
	Engine.get_singleton("EditorInterface").get_resource_filesystem().reimport_files(
		["res://addons/dialogic/Core/DialogicGameHandler.gd"]
	)
...
static func update_timer_process_callback(timer: Timer) -> void:
	timer.process_callback = (
		Timer.TIMER_PROCESS_PHYSICS if is_physics_timer() else Timer.TIMER_PROCESS_IDLE
	)
...
static func multitween(tweened_value: Variant, item: Node, property: String, part: String) -> void:
	...
	var final_value: Variant = parts.get(
		"base", item.get_meta(property + "_base_value", item.get(property))
	)
...
static func get_next_translation_id() -> String:
	ProjectSettings.set_setting(
		"dialogic/translation/id_counter",
		ProjectSettings.get_setting("dialogic/translation/id_counter", 16) + 1
	)
...
static func _set_value_in_dictionary(path: String, dictionary: Dictionary, value):
	if "." in path:
		...
		if from in dictionary.keys():
			dictionary[from] = _set_value_in_dictionary(
				path.trim_prefix(from + "."), dictionary[from], value
			)
	else:
...
static func _get_value_in_dictionary(
	path: String, dictionary: Dictionary, default = null
) -> Variant:
	if "." in path:
		var from := path.split(".")[0]
		if from in dictionary.keys():
...
static func apply_scene_export_overrides(
	node: Node, export_overrides: Dictionary, apply := true
) -> void:
	...
	for i in property_info:
		if i["usage"] & PROPERTY_USAGE_EDITOR:
			if i["name"] in export_overrides:
				if (
					str_to_var(export_overrides[i["name"]]) == null
					and typeof(node.get(i["name"])) == TYPE_STRING
				):
...
static func get_scene_export_defaults(node: Node) -> Dictionary:
	...
	if (
		Engine.get_main_loop().has_meta("dialogic_scene_export_defaults")
		and (
			node.script.resource_path
			in Engine.get_main_loop().get_meta("dialogic_scene_export_defaults")
		)
	):
		return Engine.get_main_loop().get_meta("dialogic_scene_export_defaults")[
			node.script.resource_path
		]
...
static func make_file_custom(
	original_file: String, target_folder: String, new_file_name := "", new_folder_name := ""
) -> String:
...
static func setup_script_property_edit_node(
	property_info: Dictionary, value: Variant, property_changed: Callable
) -> Control:
	...
	match property_info["type"]:
		TYPE_BOOL:
			...
			input.toggled.connect(
				DialogicUtil._on_export_bool_submitted.bind(property_info.name, property_changed)
			)
		TYPE_COLOR:
			input = ColorPickerButton.new()
			if value != null:
				input.color = value
			input.color_changed.connect(
				DialogicUtil._on_export_color_submitted.bind(property_info.name, property_changed)
			)
		...
		TYPE_INT:
			if property_info["hint"] & PROPERTY_HINT_ENUM:
				...
				input.item_selected.connect(
					DialogicUtil._on_export_int_enum_submitted.bind(
						property_info.name, property_changed
					)
				)
			else:
				input = (
					load("res://addons/dialogic/Editor/Events/Fields/field_number.tscn")
					. instantiate()
				)
				...
				input.value_changed.connect(
					DialogicUtil._on_export_number_submitted.bind(property_changed)
				)
		TYPE_FLOAT:
			input = (
		...
		TYPE_VECTOR2, TYPE_VECTOR3, TYPE_VECTOR4:
			...
			input = (
				load(
					"res://addons/dialogic/Editor/Events/Fields/field_vector" + vectorSize + ".tscn"
				)
				. instantiate()
			)
			input.property_name = property_info["name"]
			input.set_value(value)
			input.value_changed.connect(
				DialogicUtil._on_export_vector_submitted.bind(property_changed)
			)
		TYPE_VECTOR2I, TYPE_VECTOR3I, TYPE_VECTOR4I:
			...
			input.set_value(value)
			input.value_changed.connect(
				DialogicUtil._on_export_vectori_submitted.bind(property_changed)
			)
		TYPE_STRING:
			if (
				property_info["hint"] & PROPERTY_HINT_FILE
				or property_info["hint"] & PROPERTY_HINT_DIR
			):
				input = (
					load("res://addons/dialogic/Editor/Events/Fields/field_file.tscn").instantiate()
				)
				...
				if value != null:
					input.set_value(value)
				input.value_changed.connect(
					DialogicUtil._on_export_file_submitted.bind(property_changed)
				)
				...
				input.item_selected.connect(
					DialogicUtil._on_export_string_enum_submitted.bind(
						property_info.name, options, property_changed
					)
				)
			else:
				input = LineEdit.new()
				...
				input.text_submitted.connect(
					DialogicUtil._on_export_input_text_submitted.bind(
						property_info.name, property_changed
					)
				)
		TYPE_DICTIONARY:
			input = (
				load("res://addons/dialogic/Editor/Events/Fields/field_dictionary.tscn")
				. instantiate()
			)
			input.property_name = property_info["name"]
			input.set_value(value)
		...
		_:
			...
			if value != null:
				input.text = value
			input.text_submitted.connect(
				_on_export_input_text_submitted.bind(property_info.name, property_changed)
			)
	return input


static func _on_export_input_text_submitted(
	text: String, property_name: String, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(text))


static func _on_export_bool_submitted(
	value: bool, property_name: String, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(value))


static func _on_export_color_submitted(
	color: Color, property_name: String, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(color))


static func _on_export_int_enum_submitted(
	item: int, property_name: String, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(item))


static func _on_export_number_submitted(
	property_name: String, value: float, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(value))


static func _on_export_file_submitted(
	property_name: String, value: String, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(value))


static func _on_export_string_enum_submitted(
	value: int, property_name: String, list: PackedStringArray, callable: Callable
):
	callable.call(property_name, var_to_str(list[value]))


static func _on_export_vector_submitted(
	property_name: String, value: Variant, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(value))


static func _on_export_vectori_submitted(
	property_name: String, value: Variant, callable: Callable
) -> void:
	match typeof(value):
		TYPE_VECTOR2:
		...
		TYPE_VECTOR3:
			value = Vector3i(value)
		TYPE_VECTOR4:
			value = Vector4i(value)
	callable.call(property_name, var_to_str(value))


static func _on_export_dict_submitted(
	property_name: String, value: Variant, callable: Callable
) -> void:
	callable.call(property_name, var_to_str(value))
...
static func get_custom_event_defaults(event_name: String) -> Dictionary:
	if Engine.is_editor_hint():
		return ProjectSettings.get_setting("dialogic/event_default_overrides", {}).get(
			event_name, {}
		)
	else:
		if !Engine.get_main_loop().has_meta("dialogic_event_defaults"):
			Engine.get_main_loop().set_meta(
				"dialogic_event_defaults",
				ProjectSettings.get_setting("dialogic/event_default_overrides", {})
			)
...
static func get_character_suggestions(
	_search_text: String,
	current_value: DialogicCharacter = null,
	allow_none := true,
	allow_all := false,
	editor_node: Node = null
) -> Dictionary:
	var suggestions := {}
	...
	if allow_all:
		suggestions["ALL"] = {
			"value": "--All--",
			"tooltip": "All currently joined characters leave",
	...
	if editor_node:
		...
		for event_node in timeline_node.find_child("Timeline").get_children():
			if event_node == editor_node:
				break
			if (
				event_node.resource is DialogicCharacterEvent
				or event_node.resource is DialogicTextEvent
			):
		...
		for character in recent_characters:
			if character and not character.get_character_name() in suggestions:
				suggestions[character.get_character_name()] = {
					"value": character.get_character_name(),
					"tooltip": character.resource_path,
					"icon": icon.duplicate()
				}
	...
	for resource in character_directory.keys():
		suggestions[resource] = {
			"value": resource, "tooltip": character_directory[resource], "icon": icon
		}

	return suggestions


static func get_portrait_suggestions(
	search_text: String,
	character: DialogicCharacter,
	allow_empty := false,
	empty_text := "Don't Change"
) -> Dictionary:
...
static func get_portrait_position_suggestions(search_text := "") -> Dictionary:
	...
	var setting: String = ProjectSettings.get_setting(
		"dialogic/portraits/position_suggestion_names", "leftmost, left, center, right, rightmost"
	)
...
static func get_autoload_suggestions(filter: String = "") -> Dictionary:
	...
	for prop in ProjectSettings.get_property_list():
		if prop.name.begins_with("autoload/"):
			...
			suggestions[some_autoload] = {
				"value": some_autoload,
				"tooltip": some_autoload,
				"editor_icon": ["Node", "EditorIcons"]
			}
			if filter.begins_with(some_autoload):
				suggestions[filter] = {
					"value": filter, "editor_icon": ["GuiScrollArrowRight", "EditorIcons"]
				}
	return suggestions
...
static func get_autoload_script_resource(autoload_name: String) -> Script:
	...
	if autoload_name and ProjectSettings.has_setting("autoload/" + autoload_name):
		var loaded_autoload := load(
			ProjectSettings.get_setting("autoload/" + autoload_name).trim_prefix("*")
		)
	...
	if script:
		for script_method in script.get_script_method_list():
			if script_method.name.begins_with("@") or script_method.name.begins_with("_"):
				continue
			suggestions[script_method.name] = {
				"value": script_method.name,
				"tooltip": script_method.name,
				"editor_icon": ["Callable", "EditorIcons"]
			}
	...
	if script:
		for property in script.get_script_property_list():
			if property.name.ends_with(".gd") or property.name.begins_with("_"):
				continue
			suggestions[property.name] = {
				"value": property.name,
				"tooltip": property.name,
				"editor_icon": ["MemberProperty", "EditorIcons"]
			}
...
static func get_audio_channel_defaults() -> Dictionary:
	return (
		ProjectSettings
		. get_setting(
			"dialogic/audio/channel_defaults",
			{
				"":
				{
					"volume": 0.0,
...
static func validate_audio_channel_name(text: String) -> Dictionary:
	...
	if invalid_chars:
		...
		result["error_tooltip"] = (
			"Channel names cannot contain the following characters: " + "".join(invalid_chars)
		)
