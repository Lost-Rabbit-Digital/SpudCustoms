...
func list_dir(subdir := "") -> Array:
	return Array(DirAccess.get_files_at(this_folder.path_join(subdir))).map(
		func(file): return this_folder.path_join(subdir).path_join(file)
	)
...
func list_special_resources(subdir := "", extension := "") -> Dictionary:
	...
	for i in list_dir(subdir):
		if (
			extension.is_empty()
			or i.ends_with(extension)
			or (extension == ".gd" and i.ends_with(".gdc"))
		):
...
func scan_for_layout_parts() -> Array[Dictionary]:
	...
	while dir_name != "":
			...
			. append(
				{
					...
					"path":
					this_folder.path_join(dir_name).path_join(
						config.get_value("style", "scene", "")
					),
		...
		if not style_list[-1].style_path.begins_with("res://"):
			style_list[-1].style_path = this_folder.path_join(dir_name).path_join(
				style_list[-1].style_path
			)
