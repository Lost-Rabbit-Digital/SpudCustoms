...
var timeline_identifier := "":
	...
	set(value):
		...
		if (
			not timeline_identifier in DialogicResourceUtil.get_label_cache().keys()
...
func _execute() -> void:
	...
	if timeline and timeline != dialogic.current_timeline:
		dialogic.Jump.switched_timeline.emit(
			{
				"previous_timeline": dialogic.current_timeline,
				"timeline": timeline,
				"label": label_name
			}
		)
...
func from_text(string: String) -> void:
	var result := RegEx.create_from_string(r"jump (?<timeline>.*\/)?(?<label>.*)?").search(
		string.strip_edges()
	)
	if result:
...
func get_timeline_suggestions(_filter: String = "") -> Dictionary:
	...
	suggestions["(this timeline)"] = {
		"value": "", "editor_icon": ["GuiRadioUnchecked", "EditorIcons"]
	}
	for resource in DialogicResourceUtil.get_timeline_directory().keys():
		suggestions[resource] = {
			"value": resource,
			"tooltip": DialogicResourceUtil.get_timeline_directory()[resource],
...
func get_label_suggestions(_filter: String = "") -> Dictionary:
	...
	if timeline_identifier in DialogicResourceUtil.get_label_cache().keys():
		for label in DialogicResourceUtil.get_label_cache()[timeline_identifier]:
			suggestions[label] = {
				"value": label, "tooltip": label, "editor_icon": ["ArrowRight", "EditorIcons"]
			}
...
func _get_code_completion(
	...
	if symbol == " " and line.count(" ") == 1:
		CodeCompletionHelper.suggest_labels(
			TextNode, "", "\n", event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.6)
		)
		CodeCompletionHelper.suggest_timelines(
			TextNode,
