...
func post_install() -> void:
	dialogic.Settings.connect_to_change(
		"autoadvance_delay_modifier", auto_advance._update_autoadvance_delay_modifier
	)
...
func handle_input() -> void:
	...
	if not action_was_consumed:
		...
		if auto_advance.is_enabled() and auto_advance.enabled_until_user_input:
		...
		if auto_skip.disable_on_user_input and auto_skip.enabled:
...
func _input(event: InputEvent) -> void:
	if is_input_pressed(event):
		if not event is InputEventMouse:
			return
		if get_tree().get_nodes_in_group("dialogic_input").any(
			func(node): return node.is_visible_in_tree()
		):
			return
...
func is_input_pressed(event: InputEvent, exact := false) -> bool:
	var action: String = ProjectSettings.get_setting(
		_SETTING_INPUT_ACTION, _SETTING_INPUT_ACTION_DEFAULT
	)
	return (
		(event is InputEventAction and event.action == action)
		or Input.is_action_just_pressed(action, exact)
	)
...
func handle_node_gui_input(event: InputEvent) -> void:
	if Input.is_action_just_pressed(
		ProjectSettings.get_setting(_SETTING_INPUT_ACTION, _SETTING_INPUT_ACTION_DEFAULT)
	):
