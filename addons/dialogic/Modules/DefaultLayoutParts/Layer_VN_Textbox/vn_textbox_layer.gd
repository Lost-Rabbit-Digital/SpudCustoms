...
@export_subgroup("Box")
@export_file("*.tres")
var name_label_box_panel: String = this_folder.path_join("vn_textbox_name_label_panel.tres")
...
@export
var typing_sounds_mode: DialogicNode_TypeSounds.Modes = DialogicNode_TypeSounds.Modes.INTERRUPT
@export_dir
var typing_sounds_sounds_folder: String = "res://addons/dialogic/Example Assets/sound-effects/"
...
func _apply_name_label_settings() -> void:
	...
	if name_label_use_global_font_size:
		name_label.add_theme_font_size_override(
			&"font_size", get_global_setting(&"font_size", name_label_custom_font_size) as int
		)
	else:
	...
	if name_label_use_global_font and get_global_setting(&"font", false):
		name_label.add_theme_font_override(
			&"font", load(get_global_setting(&"font", "") as String) as Font
		)
	...
	if name_label_use_global_color:
		name_label.add_theme_color_override(
			&"font_color", get_global_setting(&"font_color", name_label_custom_color) as Color
		)
	else:
	...
	if ResourceLoader.exists(name_label_box_panel):
		name_label_panel.add_theme_stylebox_override(
			&"panel", load(name_label_box_panel) as StyleBox
		)
	else:
		name_label_panel.add_theme_stylebox_override(
			&"panel", load(this_folder.path_join("vn_textbox_name_label_panel.tres")) as StyleBox
		)
...
func _apply_text_settings() -> void:
	...
	if text_use_global_color:
		dialog_text.add_theme_color_override(
			&"default_color", get_global_setting(&"font_color", text_custom_color) as Color
		)
	else:
	...
	if text_use_global_font and get_global_setting(&"font", false):
		dialog_text.add_theme_font_override(
			&"normal_font", load(get_global_setting(&"font", "") as String) as Font
		)
...
func _apply_sounds_settings() -> void:
	...
	if not typing_sounds_sounds_folder.is_empty():
		type_sounds.sounds = DialogicNode_TypeSounds.load_sounds_from_path(
			typing_sounds_sounds_folder
		)
	else:
