...
const TextBubble := preload(
	"res://addons/dialogic/Modules/DefaultLayoutParts/Layer_Textbubble/text_bubble.gd"
)

var bubbles: Array[TextBubble] = []
var fallback_bubble: TextBubble = null

const textbubble_scene: PackedScene = preload(
	"res://addons/dialogic/Modules/DefaultLayoutParts/Layer_Textbubble/text_bubble.tscn"
)
...
func bubble_apply_overrides(bubble: TextBubble) -> void:
	...
	if choices_layout_force_lines:
		bubble.add_choice_container(
			VBoxContainer.new(), choices_layout_alignment, choices_custom_button, maximum_choices
		)
	else:
		bubble.add_choice_container(
			HFlowContainer.new(), choices_layout_alignment, choices_custom_button, maximum_choices
		)
