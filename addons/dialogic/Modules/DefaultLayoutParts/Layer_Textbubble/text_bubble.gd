...
func _process(delta: float) -> void:
	...
	if base_position.x > center.x:
		influence_x = -influence_x
	if base_position.y > center.y:
	...
	var offset: Vector2 = (
		Vector2.from_angle(point_a.angle_to_point(point_b))
		* bubble_rect.size
		* abs(direction.x)
		* 0.4
	)
...
func _resize_bubble(content_size: Vector2, popup := false) -> void:
	...
	name_label_holder.position = Vector2(
		0, bubble.position.y - text.position.y - name_label_holder.size.y / 2.0
	)
...
func add_choice_container(
	...
	for child: Button in choice_container.get_children():
		var prev := (
			child
			. get_parent()
			. get_child(wrap(child.get_index() - 1, 0, choice_container.get_child_count() - 1))
			. get_path()
		)
		var next := (
			child
...
func get_speaker_canvas_position() -> Vector2:
	if is_instance_valid(node_to_point_at):
		...
		if node_to_point_at is CanvasItem:
			base_position = (
				(node_to_point_at as CanvasItem).get_global_transform_with_canvas().origin
			)
