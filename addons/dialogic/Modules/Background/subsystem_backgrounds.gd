...
var default_background_scene: PackedScene = load(
	get_script().resource_path.get_base_dir().path_join(
		"DefaultBackgroundScene/default_background.tscn"
	)
)
## The default transition Dialogic will use.
var default_transition: String = get_script().resource_path.get_base_dir().path_join(
	"Transitions/Defaults/simple_fade.gd"
)
...
func load_game_state(_load_flag := LoadFlags.FULL_LOAD) -> void:
	update_background(
		dialogic.current_state_info.get("background_scene", ""),
		dialogic.current_state_info.get("background_argument", ""),
		0.0,
		default_transition,
		true
	)
...
func update_background(
	...
	argument := "",
	fade_time := 0.0,
	transition_path := default_transition,
	force := false
) -> void:
	...
	var trans_script: Script = load(
		(
			DialogicResourceUtil
			. guess_special_resource(
				"BackgroundTransition", transition_path, {"path": default_transition}
			)
			. path
		)
	)
	...
	if fade_time == 0:
		...
		trans_node.transition_finished.connect(
			_on_transition_finished.bind(background_holder, trans_node)
		)
...
func _on_transition_finished(
	background_node: DialogicNode_BackgroundHolder, transition_node: DialogicBackgroundTransition
) -> void:
	if background_node.has_meta("current_viewport"):
		if (
			background_node.get_meta("current_viewport").get_meta("node", null)
			== transition_node.next_scene
		):
...
func add_background_node(
	scene: PackedScene, parent: DialogicNode_BackgroundHolder
	...
	if not b_scene is DialogicBackground:
		printerr(
			"[Dialogic] Given background scene was not of type DialogicBackground! Make sure the scene has a script that extends DialogicBackground."
		)
		v_con.queue_free()
		viewport.queue_free()
		b_scene.queue_free()
	...
	viewport.canvas_item_default_texture_filter = ProjectSettings.get_setting(
...
func has_background() -> bool:
	return (
		!dialogic.current_state_info.get("background_scene", "").is_empty()
		or !dialogic.current_state_info.get("background_argument", "").is_empty()
	)
