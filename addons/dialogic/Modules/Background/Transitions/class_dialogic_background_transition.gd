...
func set_shader(
	path_to_shader: String = DialogicUtil.get_module_path("Background").path_join(
		"Transitions/default_transition_shader.gdshader"
	)
) -> ShaderMaterial:
	if bg_holder:
...
func tween_shader_progress(_progress_parameter := "progress") -> PropertyTweener:
	...
	var tweener := (
		tween.tween_property(bg_holder, "material:shader_parameter/progress", 1.0, time).from(0.0)
	)
