...
func build_event_editor() -> void:
	...
	add_header_edit(
		...
		{"placeholder": "Type setting", "suggestions_func": get_settings_suggestions},
		"mode != Modes.RESET_ALL"
	)
	add_header_edit(
		"_value_type",
	...
	add_header_edit(
		...
		{},
		"!name.is_empty() and (_value_type == SettingValueType.STRING or _value_type == SettingValueType.EXPRESSION) and mode == Modes.SET"
	)
	add_header_edit(
		"value",
		ValueType.NUMBER,
		{},
		"!name.is_empty()  and _value_type == SettingValueType.NUMBER and mode == Modes.SET"
	)
	add_header_edit(
		"value",
		ValueType.DYNAMIC_OPTIONS,
...
func get_settings_suggestions(filter: String) -> Dictionary:
	var suggestions := {
		filter: {"value": filter, "editor_icon": ["GDScriptInternal", "EditorIcons"]}
	}

	for prop in ProjectSettings.get_property_list():
		if prop.name.begins_with("dialogic/settings/"):
			suggestions[prop.name.trim_prefix("dialogic/settings/")] = {
				"value": prop.name.trim_prefix("dialogic/settings/"),
				"editor_icon": ["GDScript", "EditorIcons"]
			}
	return suggestions
...
func _get_code_completion(
	...
	if symbol == " " and !"reset" in line and !"=" in line and !'"' in line:
		TextNode.add_code_completion_option(
			...
			TextNode.get_theme_icon("RotateLeft", "EditorIcons")
		)
		TextNode.add_code_completion_option(
			CodeEdit.KIND_MEMBER,
			"reset all",
			"reset \n",
			event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.5),
			TextNode.get_theme_icon("ToolRotate", "EditorIcons")
		)

	if (
		(symbol == " " or symbol == '"')
		and !"=" in line
		and CodeCompletionHelper.get_line_untill_caret(line).count('"') != 2
	):
		for i in get_settings_suggestions(""):
			...
			if symbol == '"':
				TextNode.add_code_completion_option(
					...
					i,
					event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.5),
					TextNode.get_theme_icon("GDScript", "EditorIcons"),
					'"'
				)
			else:
				TextNode.add_code_completion_option(
...
func _get_start_code_completion(_CodeCompletionHelper: Node, TextNode: TextEdit) -> void:
	TextNode.add_code_completion_option(
		CodeEdit.KIND_PLAIN_TEXT, "setting", "setting ", event_color
	)
