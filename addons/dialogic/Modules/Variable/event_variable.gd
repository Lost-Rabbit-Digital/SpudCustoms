...
func _execute() -> void:
	if name:
		var original_value: Variant = dialogic.VAR.get_variable(
			name, null, operation == Operations.SET and "[" in name
		)

		if (
			value != null
			and (original_value != null or (operation == Operations.SET and "[" in name))
		):
			...
			if (
				...
				and (
					not str(original_value).is_valid_float()
					or not str(interpreted_value).is_valid_float()
				)
			):
				printerr(
					"[Dialogic] Set Variable event failed because one value wasn't a float! [",
					original_value,
					", ",
					interpreted_value,
					"]"
...
func build_event_editor() -> void:
	...
	add_header_edit(
		...
		ValueType.FIXED_OPTIONS,
		{
			...
			[
				...
				{"label": "Number", "icon": ["float", "EditorIcons"], "value": VarValueType.NUMBER},
				{
					"label": "Variable",
					"icon": load("res://addons/dialogic/Editor/Images/Pieces/variable.svg"),
					"value": VarValueType.VARIABLE
				},
				{"label": "Bool", "icon": ["bool", "EditorIcons"], "value": VarValueType.BOOL},
				{
					"label": "Expression",
	...
	add_header_edit(
		...
		{},
		"!name.is_empty() and (_value_type == VarValueType.STRING or _value_type == VarValueType.EXPRESSION) "
	)
	add_header_edit(
		"value", ValueType.BOOL, {}, "!name.is_empty() and (_value_type == VarValueType.BOOL) "
	)
	add_header_edit(
		"value", ValueType.NUMBER, {}, "!name.is_empty()  and _value_type == VarValueType.NUMBER"
	)
	add_header_edit(
		"value",
		ValueType.DYNAMIC_OPTIONS,
	...
	add_header_edit(
		"random_min",
		ValueType.NUMBER,
		{"right_text": "and", "mode": 1},
		"!name.is_empty() and  _value_type == VarValueType.RANDOM_NUMBER"
	)
	add_header_edit(
		"random_max",
		ValueType.NUMBER,
	...
	add_header_button(
		"", _on_variable_editor_pressed, "Variable Editor", ["ExternalLink", "EditorIcons"]
	)
	...
	if is_autoload or filter.count(".") == 1 and filter.split(".")[0] in autoloads:
		...
		for property in properties:
			suggestions["." + property] = {
				"value": autoload + "." + property, "editor_icon": ["MemberProperty", "EditorIcons"]
			}
...
func update_editor_warning() -> void:
	if _value_type == VarValueType.STRING and operation != Operations.SET:
		...
		if not (
			type
			in [DialogicUtil.VarTypes.INT, DialogicUtil.VarTypes.FLOAT, DialogicUtil.VarTypes.ANY]
		):
...
func _get_code_completion(
	...
	if line_until_caret.count(" ") == 1 and not "{" in line and not line_until_caret.ends_with("."):
		TextNode.add_code_completion_option(
			CodeEdit.KIND_MEMBER, "{", "{", TextNode.syntax_highlighter.variable_color
		)
		for i in autoloads:
			TextNode.add_code_completion_option(
				...
				TextNode.get_theme_icon("Node", "EditorIcons")
			)
		...
		if autoload_name in autoloads:
			...
			for i in properties.keys():
				TextNode.add_code_completion_option(
					...
					i + " ",
					event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.3),
					TextNode.get_theme_icon("MemberMethod", "EditorIcons")
				)
...
func _get_start_code_completion(_CodeCompletionHelper: Node, TextNode: TextEdit) -> void:
	TextNode.add_code_completion_option(
		...
		"set",
		"set ",
		event_color.lerp(TextNode.syntax_highlighter.normal_color, 0.5)
	)
...
func _get_syntax_highlighting(
	...
	dict = Highlighter.color_region(
		dict, Highlighter.string_color, line, '"', '"', line.find("set")
	)
	dict = Highlighter.color_region(
		dict, Highlighter.variable_color, line, "{", "}", line.find("set")
	)
	return dict
