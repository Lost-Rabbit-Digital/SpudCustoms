...
var character_identifier: String:
	...
	set(value):
		...
		if (
			...
			and ((not character) or (character and not character.portraits.has(portrait)))
		):
			portrait = ""
...
func _execute() -> void:
	...
	if character:
		...
		if portrait.is_empty():
			current_portrait = (
				dialogic
				. current_state_info["portraits"]
				. get(character.get_identifier(), {})
				. get("portrait", "")
			)

		var current_portrait_sound_mood: String = character.portraits.get(current_portrait, {}).get(
			"sound_mood", ""
		)
		dialogic.Text.update_typing_sound_mood_from_character(
			character, current_portrait_sound_mood
		)
	...
	if dialogic.has_subsystem("Styles"):
		...
		var character_style: String = (
			"" if not character else character.custom_info.get("style", "")
		)
	...
	dialogic.current_state_info["text_sub_idx"] = dialogic.current_state_info.get(
		"text_sub_idx", -1
	)
	...
	for section_idx in range(
		min(max(0, dialogic.current_state_info["text_sub_idx"]), len(split_text) - 1),
		len(split_text)
	):
		dialogic.Inputs.block_input(
			ProjectSettings.get_setting("dialogic/text/text_reveal_skip_delay", 0.1)
		)
		...
		if reveal_next_segment:
			...
			final_text = (
				ProjectSettings.get_setting("dialogic/text/dialog_text_prefix", "") + segment
			)
			...
			dialogic.Text.text_started.emit(
				{
					"text": final_text,
					"character": character,
					"portrait": portrait,
		...
		if (
			section_idx == len(split_text) - 1
			and dialogic.has_subsystem("Choices")
	...
	if dialogic.has_subsystem("History"):
		if character:
			dialogic.History.store_simple_history_entry(
				final_text,
				event_name,
				{"character": character_name_text, "character_color": character.color}
			)
		else:
...
func _try_play_current_line_voice() -> void:
	...
	if dialogic.Inputs.auto_skip.enabled and dialogic.Inputs.auto_skip.skip_voice:
		return
...
func get_character_suggestions(search_text: String) -> Dictionary:
	var suggestions := DialogicUtil.get_character_suggestions(
		search_text, character, true, false, editor_node
	)
...
func _get_code_completion(
	...
	if not ":" in line.substr(0, TextNode.get_caret_column()) and symbol == "(":
		var completion_character := (
			completion_text_character_getter_regex
			. search(line)
			. get_string()
			. trim_prefix('"')
			. trim_suffix('"')
		...
		for effect in completion_text_effects.values():
			if effect.get("arg", false):
				TextNode.add_code_completion_option(
					...
					TextNode.get_theme_icon("RichTextEffect", "EditorIcons")
				)
			else:
				TextNode.add_code_completion_option(
					CodeEdit.KIND_MEMBER,
	...
	if symbol == "=":
		if CodeCompletionHelper.get_line_untill_caret(line).ends_with("[portrait="):
			var completion_character := (
				completion_text_character_getter_regex.search(line).get_string("name")
			)
			CodeCompletionHelper.suggest_portraits(TextNode, completion_character, "]")
...
func suggest_bbcode(TextNode: CodeEdit):
	for i in [
		["b (bold)", "b"], ["i (italics)", "i"], ["color", "color="], ["font size", "font_size="]
	]:
		(
			TextNode
	...
	for i in [["new event", "n"], ["new event (same box)", "n+"]]:
		(
			...
			. add_code_completion_option(
				...
				i[1],
				TextNode.syntax_highlighter.normal_color,
				TextNode.get_theme_icon("ArrowRight", "EditorIcons"),
			)
		)
...
func load_text_effects() -> void:
	...
	if text_effects_regex.get_pattern().is_empty():
		text_effects_regex.compile(
			"(?<!\\\\)\\[\\s*/?(?<command>" + text_effects + ")\\s*(=\\s*(?<value>.+?)\\s*)?\\]"
		)
...
func _get_syntax_highlighting(
	...
	if result.get_string("text"):
		...
		for replace_mod_match in text_random_word_regex.search_all(result.get_string("text")):
			...
			dict[replace_mod_match.get_start() + result.get_start("text")] = {
				"color": Highlighter.string_color
			}
			var offset := 1
			for b: RegExMatch in RegEx.create_from_string(r"(\[[^\]]*\]|[^\/]|\/\/)+").search_all(
				replace_mod_match.get_string().trim_prefix("<").trim_suffix(">")
			):
				color.h = wrap(color.h + 0.2, 0, 1)
				dict[replace_mod_match.get_start() + result.get_start("text") + offset] = {
					"color": color
				}
				...
				offset += 1
			dict[replace_mod_match.get_end() + result.get_start("text")] = {
				"color": Highlighter.normal_color
			}
		...
		dict = Highlighter.color_region(
			dict, Highlighter.variable_color, line, "{", "}", result.get_start("text")
		)
