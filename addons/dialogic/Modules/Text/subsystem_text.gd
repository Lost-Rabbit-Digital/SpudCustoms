...
func clear_game_state(_clear_flag := DialogicGameHandler.ClearFlags.FULL_CLEAR) -> void:
	...
	set_text_reveal_skippable(
		ProjectSettings.get_setting("dialogic/text/initial_text_reveal_skippable", true)
	)
...
func update_dialog_text(text: String, instant := false, additional := false) -> String:
	...
	for text_node in get_tree().get_nodes_in_group("dialogic_dialog_text"):
		...
		if (
			text_node.enabled
			and (text_node == text_node.textbox_root or text_node.textbox_root.is_visible_in_tree())
		):
			if instant:
				text_node.text = text
				...
				if current_character:
					var character_prefix: String = current_character.custom_info.get(
						DialogicCharacterPrefixSuffixSection.PREFIX_CUSTOM_KEY,
						DialogicCharacterPrefixSuffixSection.DEFAULT_PREFIX
					)
					var character_suffix: String = current_character.custom_info.get(
						DialogicCharacterPrefixSuffixSection.SUFFIX_CUSTOM_KEY,
						DialogicCharacterPrefixSuffixSection.DEFAULT_SUFFIX
					)
			...
			dialogic.current_state_info["text_parsed"] = (
				(text_node as RichTextLabel).get_parsed_text()
			)
...
func _on_dialog_text_finished() -> void:
	text_finished.emit(
		{
			"text": dialogic.current_state_info["text"],
			"character": dialogic.current_state_info["speaker"]
		}
	)
...
func hide_textbox(instant := false) -> void:
	...
	if (
		!emitted
		and !get_tree().get_nodes_in_group("dialogic_dialog_text").is_empty()
		and get_tree().get_nodes_in_group("dialogic_dialog_text")[0].textbox_root.visible
	):
...
func is_textbox_visible() -> bool:
	return get_tree().get_nodes_in_group("dialogic_dialog_text").any(
		func(x): return x.textbox_root.visible
	)
	...
	for next_indicator in get_tree().get_nodes_in_group("dialogic_next_indicator"):
		if next_indicator.enabled:
			if (
				(
					question
					and "show_on_questions" in next_indicator
					and next_indicator.show_on_questions
				)
				or (
					autoadvance
					and "show_on_autoadvance" in next_indicator
					and next_indicator.show_on_autoadvance
				)
				or (!question and !autoadvance)
			):
				next_indicator.show()
		else:
			next_indicator.hide()
...
func update_text_speed(
	letter_speed: float = -1,
	absolute := false,
...
func set_text_reveal_skippable(skippable := true, temp := false) -> void:
	if !dialogic.current_state_info.has("text_reveal_skippable"):
		dialogic.current_state_info["text_reveal_skippable"] = {
			"enabled": false, "temp_enabled": false
		}
...
func is_text_reveal_skippable() -> bool:
	return (
		dialogic.current_state_info["text_reveal_skippable"]["enabled"]
		and dialogic.current_state_info["text_reveal_skippable"].get("temp_enabled", true)
	)
	...
	for indexer in DialogicUtil.get_indexers(true):
		for effect in indexer._get_text_effects():
			...
			if effect.has("subsystem") and effect.has("method"):
				text_effects[effect.command]["callable"] = Callable(
					dialogic.get_subsystem(effect.subsystem), effect.method
				)
			elif effect.has("node_path") and effect.has("method"):
				text_effects[effect.command]["callable"] = Callable(
					get_node(effect.node_path), effect.method
				)
			else:
				continue
			text_effect_names += effect.command + "|"
	text_effects_regex.compile(
		(
			"(?<!\\\\)\\[\\s*(?<command>"
			+ text_effect_names.trim_suffix("|")
			+ ")\\s*(=\\s*(?<value>.+?)\\s*)?\\]"
		)
	)
...
func parse_text_effects(text: String) -> String:
	...
	for effect_match in text_effects_regex.search_all(text):
		...
		bbcode_correction = (
			effect_match.get_start() - position_correction - len(rtl.get_parsed_text())
		)
		# append [index] = [command, value] to effects dict
		parsed_text_effect_info.append(
			{
				"index": effect_match.get_start() - position_correction - bbcode_correction,
				"execution_info": text_effects[effect_match.get_string("command")],
				"value": effect_match.get_string("value").strip_edges()
			}
		)

		text = (
			text.substr(0, effect_match.get_start() - position_correction)
			+ text.substr(
				effect_match.get_start() - position_correction + len(effect_match.get_string())
			)
		)
	...
	for indexer in DialogicUtil.get_indexers(true):
		for modifier in indexer._get_text_modifiers():
			if modifier.has("subsystem") and modifier.has("method"):
				text_modifiers.append(
					{
						"method":
						Callable(dialogic.get_subsystem(modifier.subsystem), modifier.method)
					}
				)
			elif modifier.has("node_path") and modifier.has("method"):
				text_modifiers.append(
					{"method": Callable(get_node(modifier.node_path), modifier.method)}
				)
...
func color_character_names(text: String) -> String:
	...
	for result in color_regex.search_all(text):
		text = text.insert(
			result.get_start("name") + ((9 + 8 + 8) * counter),
			"[color=#" + character_colors[result.get_string("name")].to_html() + "]"
		)
...
func collect_character_names() -> void:
	...
	for dch_identifier in DialogicResourceUtil.get_character_directory():
		var character := (
			DialogicResourceUtil.get_character_resource(dch_identifier) as DialogicCharacter
		)

		if character.display_name:
			if "{" in character.display_name and "}" in character.display_name:
				character_colors[dialogic.VAR.parse_variables(character.display_name)] = (
					character.color
				)
			else:
...
func modifier_autopauses(text: String) -> String:
	...
	for i in _autopauses.keys():
		...
		for result in i.search_all(text):
			if absolute:
				text = text.insert(
					result.get_end() + offset, "[pause=" + str(_autopauses[i]) + "!]"
				)
