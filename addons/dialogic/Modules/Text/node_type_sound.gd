...
func _on_started_revealing_text() -> void:
	if !enabled or (get_parent() is DialogicNode_DialogText and !get_parent().enabled):
		return
	characters_since_last_sound = (
		current_overwrite_data.get("skip_characters", play_every_character - 1) + 1
	)
...
func _on_continued_revealing_text(new_character: String) -> void:
	...
	if !Engine.is_editor_hint() and get_parent() is DialogicNode_DialogText:
		if (
			DialogicUtil.autoload().has_subsystem("Voice")
	...
	audio_player.stream = current_overwrite_data.get("sounds", sounds)[RNG.randi_range(
		0, current_overwrite_data.get("sounds", sounds).size() - 1
	)]
	...
	audio_player.pitch_scale = max(
		...
		(
			...
			+ (
				current_overwrite_data.get("pitch_variance", pitch_variance)
				* RNG.randf_range(-1.0, 1.0)
			)
		)
	)
	audio_player.volume_db = (
		current_overwrite_data.get("volume_base", base_volume)
		+ (
...
func load_overwrite(dictionary: Dictionary) -> void:
	...
	if dictionary.has("sound_path"):
		current_overwrite_data["sounds"] = DialogicNode_TypeSounds.load_sounds_from_path(
			dictionary.sound_path
		)
...
static func load_sounds_from_path(path: String) -> Array[AudioStream]:
	...
	for file in DialogicUtil.listdir(path, true, false, true, true):
		if !file.ends_with(".import"):
			continue
		if (
			file.trim_suffix(".import").get_extension().to_lower() in ["mp3", "wav", "ogg"]
			and ResourceLoader.load(file.trim_suffix(".import")) is AudioStream
		):
