...
func save(
	slot_name := "",
	is_autosave := false,
	thumbnail_mode := ThumbnailMode.TAKE_AND_STORE,
	slot_info := {}
) -> Error:
...
func load(slot_name := "") -> Error:
	...
	if set_latest_error:
		push_error(
			(
				"[Dialogic Error]: Failed to store latest slot to global info. Error %d '%s'"
				% [set_latest_error, error_string(set_latest_error)]
			)
		)
...
func save_file(slot_name: String, file_name: String, data: Variant) -> Error:
	...
	if encryption_password.is_empty():
		file = FileAccess.open(
			SAVE_SLOTS_DIR.path_join(slot_name).path_join(file_name), FileAccess.WRITE
		)
	else:
		file = FileAccess.open_encrypted_with_pass(
			SAVE_SLOTS_DIR.path_join(slot_name).path_join(file_name),
			FileAccess.WRITE,
			encryption_password
		)
	...
	if file:
		...
		push_error(
			(
				"[Dialogic Error]: Could not save slot to file. Error: %d '%s'"
...
func set_global_info(key: String, value: Variant) -> Error:
	...
	if encryption_password.is_empty():
		...
		var load_error := global_info.load_encrypted_pass(
			SAVE_SLOTS_DIR.path_join("global_info.txt"), encryption_password
		)
		if load_error:
			printerr("[Dialogic Error]: Couldn't access global saved info file.")
			return load_error
			...
			global_info.set_value("main", key, value)
			return global_info.save_encrypted_pass(
				SAVE_SLOTS_DIR.path_join("global_info.txt"), encryption_password
			)
...
func get_global_info(key: String, default: Variant) -> Variant:
	...
	if encryption_password.is_empty():
		...
		global_info.load_encrypted_pass(
			SAVE_SLOTS_DIR.path_join("global_info.txt"), encryption_password
		)
		== OK
	):
...
func get_encryption_password() -> String:
	if (
		OS.is_debug_build()
		and ProjectSettings.get_setting("dialogic/save/encryption_on_exports_only", true)
	):
		return ""
	...
	if DirAccess.dir_exists_absolute(path):
		...
		while not file_name.is_empty():
			...
			if remove_error:
				push_warning(
					(
						"[Dialogic Error]: Encountered error while removing '%s': %d\t%s"
						% [path.path_join(file_name), remove_error, error_string(remove_error)]
					)
				)
			file_name = directory.get_next()
