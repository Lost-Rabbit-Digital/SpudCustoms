...
func get_container(position_id: String) -> DialogicNode_PortraitContainer:
	for portrait_position: DialogicNode_PortraitContainer in get_tree().get_nodes_in_group(
		&"dialogic_portrait_con_position"
	):
...
func get_container_container() -> CanvasItem:
	var any_portrait_container := get_tree().get_first_node_in_group(
		&"dialogic_portrait_con_position"
	)
...
func add_container(position_id: String) -> DialogicNode_PortraitContainer:
	...
	if example_position:
		...
		position_changed.emit(
			{&"change": "added", &"container_node": new_position, &"position_id": position_id}
		)
		return new_position
	return null
...
func move_container(
	...
	if destination_container:
		...
		target_position = (
			destination_container.position + destination_container._get_origin_position()
		)
...
func copy_container_setup(
	from: DialogicNode_PortraitContainer, to: DialogicNode_PortraitContainer
) -> void:
...
func translate_container(
	...
	if !container.has_meta(&"default_translation"):
		container.set_meta(
			&"default_translation", container.position + container._get_origin_position()
		)

	var final_translation: Vector2
	if typeof(translation) == TYPE_STRING:
		final_translation = str_to_vector(
			translation, container.position + container._get_origin_position()
		)
	...
	if tween:
		tween.tween_method(
			DialogicUtil.multitween.bind(container, "position", "base"),
			container.position,
			final_translation,
			time
		)
...
func resize_container(
	...
	var relative_position_change := (
		container._get_origin_position() - container._get_origin_position(final_rect_resize)
	)

	if tween:
		tween.tween_method(
			DialogicUtil.multitween.bind(container, "position", "resize_move"),
			Vector2(),
...
func load_position_container(position_id: String) -> DialogicNode_PortraitContainer:
	...
	if (
		not dialogic.current_state_info.has("portrait_containers")
		or not dialogic.current_state_info.portrait_containers.has(position_id)
	):
		return null
...
func str_to_vector(input: String, base_vector := Vector2()) -> Vector2:
	var vector_regex := RegEx.create_from_string(
		r"(?<part>x|y)\s*(?<number>(-|\+)?(\d|\.|)*)(\s*(?<type>%|px))?"
	)
	var vec := base_vector
		...
		match i.get_string(&"type"):
			...
			"%", _:
				match i.get_string(&"part"):
					"x":
						value *= get_viewport().get_visible_rect().size.x
					"y":
						value *= get_viewport().get_visible_rect().size.y
...
func reset_container(
	container: DialogicNode_PortraitContainer, time := 0.0, tween: Tween = null
) -> void:
	if container.has_meta(&"default_translation"):
		translate_container(
			container, vector_to_str(container.get_meta(&"default_translation")), false, tween, time
		)
	if container.has_meta(&"default_rotation"):
		rotate_container(container, container.get_meta(&"default_rotation"), false, tween, time)
	if container.has_meta(&"default_size"):
		resize_container(
			container, vector_to_str(container.get_meta(&"default_size")), false, tween, time
		)
