...
static func guess_animation(string: String, type := AnimationType.ALL) -> String:
	...
	return (
		...
		. guess_special_resource(&"PortraitAnimation", string, default, filter, ignores)
		. get("path", "")
	)
...
static func get_suggestions(
	_search_text := "", current_value := "", empty_text := "Default", action := AnimationType.ALL
) -> Dictionary:
	var suggestions := {}
