...
var channel_name := "":
	set(value):
		if channel_name != channel_name_regex.sub(value, "", true):
			...
			var defaults: Dictionary = DialogicUtil.get_audio_channel_defaults().get(
				channel_name, {}
			)
			if defaults:
...
var regex := (
	RegEx
	. create_from_string(
		r'(?:audio)\s*(?<channel>[\w-]{2,}|[\w]*)?\s*(")?(?<file_path>(?(2)[^"\n]*|[^(: \n]*))(?(2)"|)(?:\s*\[(?<shortcode>.*)\])?'
	)
)
...
func store_to_shortcode_parameters(params: Dictionary = {}) -> String:
	...
	for parameter in params.keys():
		...
		var default_value: Variant = custom_defaults.get(
			parameter_info.property, parameter_info.default
		)
		...
		if channel_name in channel_defaults.keys():
			default_value = channel_defaults[channel_name].get(
				parameter_info.property, default_value
			)

		if typeof(value) == typeof(default_value) and value == default_value:
			if (
				not "set_" + parameter_info.property in self
				or not get("set_" + parameter_info.property)
			):
				continue
		...
		result_string += (
			...
			+ parameter
			+ '="'
			+ value_to_string(value, parameter_info.get("suggestions", Callable()))
			+ '"'
		)
...
func build_event_editor() -> void:
	...
	add_body_edit(
		...
		{"editor_icon": ["FadeCross", "EditorIcons"], "tooltip": "Overwrite Fade Length"},
	...
	add_body_edit(
		"set_volume",
		ValueType.BOOL_BUTTON,
		{"editor_icon": ["AudioStreamPlayer", "EditorIcons"], "tooltip": "Overwrite Volume"},
		"!file_path.is_empty() and has_channel_defaults()"
	)
	add_body_edit(
		"volume",
		ValueType.NUMBER,
		{"left_text": "Volume:", "mode": 2},
		"!file_path.is_empty() and (not has_channel_defaults() or set_volume)"
	)
	add_body_edit(
		"set_audio_bus",
		ValueType.BOOL_BUTTON,
		{"editor_icon": ["AudioBusBypass", "EditorIcons"], "tooltip": "Overwrite Audio Bus"},
		"!file_path.is_empty() and has_channel_defaults()"
	)
	add_body_edit(
		"audio_bus",
	...
	add_body_edit(
		"set_loop",
		ValueType.BOOL_BUTTON,
		{"editor_icon": ["Loop", "EditorIcons"], "tooltip": "Overwrite Loop"},
	...
	add_body_edit(
		"set_sync_channel",
		ValueType.BOOL_BUTTON,
		{"editor_icon": ["TransitionSync", "EditorIcons"], "tooltip": "Enable Syncing"},
	...
	add_body_edit(
		...
		ValueType.DYNAMIC_OPTIONS,
		{
			...
			"validation_func": DialogicUtil.validate_audio_channel_name,
			"tooltip":
			"Use an existing channel or type the name for a new channel. If channel doesn't exist, this setting will be ignored.",
		},
...
func get_audio_channel_suggestions(filter: String) -> Dictionary:
	...
	suggestions["(One-Shot SFX)"] = {
		"value": "",
		"tooltip":
		"Used for one shot sounds effects. Plays each sound in its own AudioStreamPlayer.",
...
func _get_code_completion(
	CodeCompletionHelper: Node, TextNode: TextEdit, line: String, word: String, symbol: String
) -> void:
...
func _get_syntax_highlighting(
	...
	if result.get_string("shortcode"):
		...
		dict = Highlighter.color_shortcode_content(
			dict, line, result.get_start("shortcode"), 0, event_color
		)
